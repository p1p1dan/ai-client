...
## Appendix B — Incident lessons (added 2026-09-14)

### B1. Tests never signal real processes with a fabricated pid
- **Incident (2026-09-14):** a host test exercised the POSIX branch of the process-tree killer with a fake child `{ pid: 1 }`, expecting `EPERM`. But `process.kill(-1, 'SIGKILL')` does not mean "the group led by pid 1": POSIX defines pid −1 as *every process the caller is allowed to signal*. Each run of `host.test.ts` therefore killed the developer's entire desktop session (`systemd --user`, the terminal, the editor, the agent itself), which presented as "the machine locks and every window is gone". It was misdiagnosed three times as memory exhaustion on the 2-core / 3.3 GB box; the lock file and worker caps added for that changed nothing.
- **Rule:** production code that calls `process.kill` / `kill(-pgid)` MUST take the signalling function as an injectable dependency (the tree killer has `killGroup`, next to `spawnProcess`). A test that reaches such code with a fabricated pid MUST inject a stub or `vi.spyOn(process, 'kill')` before the first call. pid values 1, 0 and −1 are never valid inputs to a real signal from a test.
- **Diagnosis:** when "the machine died" during a test run, read the journal before blaming resources. `user@1000.service: Main process exited, code=killed, status=9/KILL` with a `node (vitest N)` survivor is a broadcast kill from inside the suite (the caller is the only process `kill(-1)` spares). `journalctl -u systemd-oomd` and `journalctl -k | grep -i oom` are both empty in that case.
- **Acceptance:** a guard test that spies on `process.kill` and asserts the default path is the only one that reaches it; `grep -rn "process\.kill(" src --include=*.test.ts` shows nothing beyond signal-0 liveness probes. Task briefs for anything touching process cleanup carry this rule verbatim.
