...
| **T-19** | 消息队列（运行中解禁输入 + FIFO 排队；提案出自团队轨道会话，2026-07-24 转述；**2026-07-30 复活并落地 `1b350ff`**） | 原始占位段落（append-only 保留）：『提案出自团队轨道会话（用户转述 2026-07-24），尚未落库——请团队会话把提案内容补进本行与团队台账（推测方向：turn 运行中排队后续消息，CC 原生有 `queue-operation` 机制可依托；以团队定稿为准）。落库前不排期』。**2026-07-30 复活**：open-questions #18 拍板 A（解禁打字 + 后发消息排队，等当前回合完再处理）触发本任务从「等团队定稿」转为「主线设计施工」当日完成。**研究**（Explore 委派）：复刻 openchamber 冻结版四件套语义——输入框运行中从不禁用、回车默认入队、纯客户端 FIFO（每会话上限 20）、idle 触发自动放行；挂起问答/授权时发送 = 自动取消挂起卡 + 入队（直发会被运行中回合静默吞，openchamber 注释明写的坑）；四件全客户端可实现，协议零改动。**设计**（deep-reasoner）：8 决策 + 14 行边界表规格（scratchpad `t19-design.md`，487 行）。**落地**（`1b350ff`，14 文件 +2453/-83）：纯函数层 `messageQueue.ts`/`queueRelease.ts` + `stores/messageQueue.ts` 薄壳；`ChatComposer` 运行中解禁输入、`runSend` 改**提交即消费**（草稿与附件在提交点消费，全部失败出口不吞字）；`QueuedMessageStrip` 队列条（可删除、可拖序 = 交换语义）；Stop→队列暂停，Resume/新回合恢复；`useQueueRelease` 挂 idle 自动放行。**对抗复核位变更**：Codex 通道连败两次（容量满载），改派 deep-reasoner，产出 1 blocker/6 major/17 minor，按 R5 预案回退「失败载荷归队」至组件局部 retryable 形态等处理，详见主线台账 2026-07-30 T-19 行。**已知代价（设计决策非缺陷）**：后台会话不自动放行（需切回前台）；应用重启队列丢失（纯内存态）。 | ① 运行中输入框不禁用，可打字可粘贴图；② 回车入队显示 Composer 上方可删除/可拖序的队列条；③ idle 时队首自动放行、FIFO 依次消费；④ 挂起问答/授权时发送 = 自动取消挂起卡 + 入队，不直发不误代答；⑤ Stop 后队列暂停，Resume/新回合自动恢复放行；⑥ 纯函数层单测覆盖；⑦ 三绿。**✅ 2026-07-30 代码落地 `1b350ff`**（三批施工 + 对抗复核（Codex 两次容量满载改派 deep-reasoner）1 blocker/6 major/17 minor 全部处理 + 修复批；+100 例总 1097 三绿。**GUI 点验待用户**，清单见主线台账 2026-07-30 T-19 行） | 无（研究+设计+施工同日完成） | 待定 |
...
| **T-29** | assistant 正文 Markdown 渲染（**D26**，2026-07-30 立项，实为补交付——ARD §9 已标 ✅） | 方案一：复用依赖树内 `react-markdown@10`+`remark-gfm`+`remark-breaks`+`shiki`（仓内成熟用法 `files/MarkdownPreview.tsx:208-217` / `CodeReviewModal.tsx:371-376` 可参）；**chat 专属 components 映射，不复用 MarkdownPreview 那套**（其本地文件 URL/相对图片解析对 LLM 输出是攻击面）；安全清单：不引 rehype-raw、链接仅 http(s) 且走 `shell.openExternal`、**一期不渲染远程图片**（防外部信标）；流式策略：流式期纯文本、`message.completed` 后切 markdown（与 ARD 既有口径一致）；观感对齐 Flexoki token（代码块走 shiki 主题同步） | ① assistant 文本的标题/列表/行内代码/代码块/链接正确渲染且零 XSS 面（无 rehype-raw、无远程图）；② 流式期无渲染抖动（完成后切换）；③ 代码块高亮与主题一致；④ 逻辑可测部分下沉纯函数；⑤ 三绿 | T-05 ✅（时间线新形态） | ≈2d。**✅ 2026-08-04 代码结项 `d320206`+`666c7c3`+`4507df3`**（前两 hash 曾提交并推送但全套状态文档漏记，本日补登；双轨对抗复核闭环：Workflow 5 视角 27 findings / 3 证伪、verify:security 独立补格全 upheld 并追加最长行近二次方第四发现、Codex 双盲收活枚举 20 renderer 关闭 (e)；修复批 13 文件——流式门下沉 `deriveStreamingBlockIds` 根治 major 会话级误伤并把 (c) 收窄为逐消息即时转正、F-C7 三重高亮预算、64 项 LRU、`__proto__` 守卫、href 主机名加固、共享 TS 编译器 API stripComments 换掉全仓四份假绿拷贝；三绿 vitest 1901 例仅 3 例 Windows-only + 产物 20 个 shiki chunk 全在白名单；红线 `chatSessions.ts` 三提交零改动）。**✅ 2026-08-04 GUI 点验用户验收**（首轮唯一缺陷「内容不可选中」当轮修复 `b08f6ae`；拍板两项裁定维持现状：脚注区 13px / 六级标题三档），明细见主线台账 2026-08-04 T-29 三行 |
| **T-30** | 观感打磨：分域字体落地 + 审计批次 1+2（**D25/D26**，2026-07-30 立项） | 依据 [`polish-audit-20260730.md`](../design/polish-audit-20260730.md)（34 条清单）：**批次 1 快赢 12 项**（删 USER 角标（回归基线）/user 气泡回归 A07/修 ModelSelect 默认值不落盘致 meta 丢 model 名的真 bug/`--tool-arg` 62%→78%/段头回归/接上 letter-spacing 梯度等，零基线修订）；**批次 2**（D25 分域字体：`--font-sans` 比例 + `--font-mono` 专供代码/路径/分支，含 design-system.md 字体节改写与两基线标注修订；D26 四条 A07 修订实施：失败行分级红/失败自动展开簇规则/侧栏 chip 放宽/气泡满宽；错误红字符 -86% 目标；排版层级 weight×tracking×color 三件套接通）；**与 T-23 交叠三项（MainHeader 死按钮/假环）不碰仍归 T-23** | ① 审计批次 1+2 清单逐项闭环（每项在审计文档标 ✅/偏离说明）；② 分域字体后中英混排/侧栏截断实测改善（对照截图入库）；③ A07/phase0a/design-system 三文档与代码同步（账实一致）；④ 纯函数单测；⑤ 三绿 + 用户 GUI 验收「协调感」。**✅ 2026-07-30 批次 1 代码落地 `3dcd2dc`**（6 文件 +95/-40；12 项快赢中 10 项落地——P-08 删 USER 角标/P-09 user 气泡回归 A07/P-11 NoticeMessage/P-06 失败框收敛/P-17 间距/P-05 destructive 降 70% color-mix/`--tool-arg` 62%→78%/P-14 模型名兜底/QuestionCard+LeftNav 字距梯度；**P-19/P-22 让给 T-23**，口径不变不重复登记；三绿：typecheck 干净/lint 672 文件 0 错误(外来 a08 html 29 警告不计)/vitest 71 文件 1097 例，与 T-19 同批验证、批1 本身无新增测试）。**✅ 2026-08-03 批次 2 代码落地 `9e2736b`**（70 文件 +4202/−603，测试 1345→1415；D25 分域字体全量 + 形态三拍板①②③ + 第四轮追补合并控件；规格链=[`2026-07-30-d25-font-domain-design.md`](2026-07-30-d25-font-domain-design.md)+[`2026-07-31-t30b2-composer-form-design.md`](2026-07-31-t30b2-composer-form-design.md)+[`-addendum-round4.md`](2026-07-31-t30b2-composer-form-addendum-round4.md)；`a09-font-domain-baseline.html` 已建；编排者三条临时裁定已随第二轮点验无异议生效并落地；Opus+Codex 双轨对抗复核 2+4 major 全闭环，pill 落法 as-built 修正为静息半高 rounded-[21px]；A07 v4 追记一~十一节）。**GUI 点验待用户**（第五轮，清单见 plantree implementation-status 用户线 0-nonies），明细见主线台账 2026-08-03 行。 | D25/D26 ✅ · T-05/T-28 ✅ | 批1 ≈1d + 批2 ≈3d |
| **T-31** | 回复解剖（回合分组 / 状态段 / 折叠壳 / 尾部状态条）+ 置顶用户气泡（第四轮点验第 6/7 条，2026-08-03 开工；排 T-30 批2 后、T-29 前） | **施工规格权威 = [`2026-07-31-reply-anatomy-design.md`](2026-07-31-reply-anatomy-design.md)**（本行只留摘要）：时间线引入 `ChatTurn` 回合层——纯函数 `groupMessagesIntoTurns`（`chatTurn.ts` 新建，零 store 改动、红线 `chatSessions.ts` 零改动）；状态读条按「描述在飞回合归回合 / 描述手上草稿归 Composer」切分，回合侧文案仍由 `composerSendingLine()` 同源生成（新建 `turnStatus.ts` 委托，不复制字符串）；`Worked for Ns ⌄` 展开体由统计文本改判为**过程段**（Thinking+工具行+中间正文，统计移入行内 arg；A07 v5 追记）；`splitTurnBody` 切 process/answer，answer 恒可见；折叠默认态判定表含**授权卡强制展开安全红线**（最高优先级）；置顶气泡 = 就地 CSS sticky（band `py-2.5 bg-background`，回合 `<section>` 为 containing block，零滚动监听）+ ScrollArea 按边 fade（`scrollFade="bottom"` 向后兼容加法）+ `scroll-state()` 钉住态 3 行截断（Electron 39 支持属推断，GUI 实测不可用降 line-clamp）；尾部状态条 = 模型 · 相对时间（P-18）· copy（P-34，`buildTurnCopyText` 只拼正文块不含工具 IN/OUT）；聚合行补 `ran N command(s)`；**§9 六项可拍板点按推荐值开工**（δ footer 不显耗时 / ε `Generating · Ns` / ζ 失败块不移 / η scroll-state / θ 按边 fade / ι 不做 fork，编排者执行裁定、用户点验时可否决回改）；有意不对齐清单见规格 §10（fork/👍👎/暗色工具卡等六项） | ① F-B1~F-B15 纯函数断言全绿（含 F-B1 零丢消息不变量 `Σbody+Σuser===messages.length`、F-B3/F-B4 授权卡防折叠安全红线、F-B9 间距算术 10+10=20、F-B11 状态不两处并显）；② 收起/展开态与 Cursor round-4 参照图（`refs/feedback-20260731-round4/`）对齐，默认态按 §4.3 判定表；③ 状态读条迁移后 Composer 信息不丢核对表（§3.3）全 ✅；④ 四项不可断言点（sticky 生效 / mask 不吃钉住气泡 / scroll-state 可用性 / defaultOpen 挂载期求值）注释入码 + GUI 点验；⑤ A07 v5 追记（Worked-for 语义修订 / 折叠与 D26② 两级嵌套 / 置顶气泡 / 间距构成 10+10）+ `MessageTimeline.tsx` V-b 注释更新 + design-system 连带；⑥ 三绿（Linux 口径） | T-30 批2 ✅ `9e2736b` · D26④ **于 T-31 补落**（原写 ✅ 系账实不符，T-30 批2 名义含此项实未落地，2026-08-03 更正） · D25 字号档 ✅ | ≈3.6d（R0~R6，§8.3）。**✅ 2026-08-03 代码落地 `8109d45`**（36 文件 +4712/−318，测试 1593→1759；双轨对抗复核 + 修复批三轮 + Codex 终验两轮全闭环；§4.5 `ran N command(s)` 撤回、恢复历史头槽降级链两处 as-built 偏离已登记；三绿 + 产物含 scroll-state 规则；红线零改动）。**GUI 点验待用户（清单 0-undecies）**，明细见主线台账 2026-08-03 T-31 行 |
...
| **T-32** | 右栏骨架回归 A08（**D27**，2026-08-05 立项） | **开工前置已解除**（open-q #28 于 2026-08-05 当日用户裁定关闭）。**施工规格权威 = [`2026-08-05-t32-a08-shell-regression-spec.md`](2026-08-05-t32-a08-shell-regression-spec.md)**（五切片 S1~S5 + 六风险）。回归面（对照 [T-12~T-15 规格 §7](2026-08-04-t12-15-surface-spec.md)「被取代」十一项）：① `ContextPanel` 恢复 **tab 条**；② `ContextPanelRail` 改 **仅 panel 收起时渲染**（联动收展；展开时右缘无图标，L0 阈值因此不含 44px）；③ `MainHeader` 恢复**贯通中右**（与 T-23 刚落的单行 h-9 三顶条齐平口径**直接冲突，须一并重定**）；④ 恢复 **L0/L1/L2 画幅降级梯**（1580 = 280+400+520+380 / 1244 = 280+400+520+44）与 `manualPanel`/`manualChat` 会话内手动覆盖（关文件清零）；⑤ **editor 回中列（已裁定）**：`EditorSurfaceView` 从右栏搬进中列成 `chat ║ editor` 并排（`a08:1208-1241`），补 `ed-grip` 比例拖拽与 `editorRatio`、editor head（文件名 + 未保存点 + 「隐去 chat」钮 + 关闭文件钮）；**右栏 files tab 降为纯文件树**，点文件在中列打开（不再开 panel 内 tab）；关文件 → editor 撤列、chat 回、panel 恢复 `panelOpen`。⑥ **右栏 tab 四项** `git | files | context | terminal`，Rail 同步四图标 + git-only dot；`` Ctrl+` `` 改为打开/聚焦 terminal tab（非 dock 语义）。**三项豁免不做**（D27）：terminal 不回 BottomDock（维持 surface，`BottomDock.tsx` 不复活）· context 不加 Quick notes/Todo/Plans · git 不加 branch/pr/sync/stash。**沿用不变**：宽度记忆 380–1400 · git-only dot · 44px Rail · 250ms 过渡（A08 与现状本就一致）。**一处刻意不照搬 A08（A06 精神，编排者裁定，非用户拍板）**：**editor 保留多 tab**（现有 `EditorTabs`）——A08 画单文件，但隐藏 tab = 隐藏脏文件会丢改动；§7 原裁定沿用。 | ② 回归后 A08 的 tab 切换 / 收展 / 降级梯 / 手动覆盖四类状态迁移与 A08 §「状态机」表逐行吻合；③ 布局与 surface 选择逻辑**仍下沉纯函数**（不得回退成组件内联，`shellLayoutModel.ts` 现有断言面只增不减）；④ keep-alive 契约不回归（terminal 跑 `sleep 30` 切 tab 再切回，进程仍在、输出连续）；⑤ 三绿 + GUI 点验 | D27 · T-22 as-built · T-23（顶栏口径冲突，须一并重定）· ~~open-q #28~~（2026-08-05 已关闭） | **3d 级**（editor 回中列已裁定） |
| **T-33** | 网络重试横幅（2026-08-05 立项，源自 2026-08-04 ACP 调研的三条能力缺失之一） | **数据已全通，只差 UI**：`stores/chatSessions.ts:85` 的 `retry?: SessionRetryInfo` 已由 Host 的 transport-retry 回合写入，且在**无 retry 载荷的 `session.status` 上会被清回 undefined**（该清除语义已存在，勿重写）。本任务只做呈现：回合区顶部一条非致命横幅，表达「不是卡死了，正在重试」+ 第几次/下次间隔（取 `SessionRetryInfo` 现有字段，缺字段即不渲染该段，A06）。**UI 逻辑下沉纯函数**（`deriveRetryBanner`），`.tsx` 零覆盖是本仓既有约束。 | ① 真实重试回合出现横幅、重试结束自动消失；② 无 retry 时零渲染（不留空盒）；③ 纯函数覆盖 SessionRetryInfo 各字段缺失组合；④ 三绿。**✅ 2026-08-05 代码落地 `ba6b371` + Codex 复核修复 `e602096`+`d3635d3`+`8bf4829`（终验两轮）**（as-built：横幅双槽挂载——`ChatTurn` turnBody 首子 + `PendingTurnHead` 上方（覆盖握手窗口，单槽互斥由 prop 收窄保证）；**门语义经复核推翻重做**：首版 `hasBlocks` 单调门会永久压制工具调用后的 mid-turn 重试（Codex F1 major，原设计与测试锁一并改写），现为 **epoch 进度戳门**——retry 对象引用即 epoch、`useMemo` 捕获当刻进度戳（**块数+已流字符**，终验第二轮指出恢复可续写既有 text block 而不长块数），「本次 retry 之后有新进度」才判过期（同 commit 批处理窗口为已接受残留，注释入码）；验收③的「缺字段」在实际链路是哨兵值而非 undefined（`eventNormalizer` 防御回填 0/0/0/null/unknown），纯函数按哨兵语义判缺段且测试双轨覆盖；**执行裁定：与 `composerSendingLine` 既有 `· Network retry N/M` 后缀共存**（最小侵入；同一事实两显仅 awaiting+retry 一态，点验④可否决）；视觉走 `status-running` 三件套（HostStatusBanner 既定「运行中横幅」写法）。红线零改动。**GUI 点验待用户（0-septendecies）**） | 无（数据已在红线 store，**只读不改**） | 0.5d |
| **T-34** | 子 agent 实况（2026-08-05 立项，三条能力缺失中唯一有产品价值增量的一条） | **现状**：`forwardSubagentText`（`src/agent-host/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts:1619`，我们装的 `0.3.218` 已有）**默认 false**，故子 agent 的**文本收不到**；但其 `tool_use`/`tool_result` **会收到且被当成主 agent 干的**——Claude 跑 Task 时子 agent 的 Read/Grep/Bash 显示成主 agent 的工具行（既有缺陷）。**四件事的分量差极大**：拉起（Claude 自己做，我们不碰）· 传输（开一个布尔值 + 协议加可选字段透传 `parent_tool_use_id`，纯加法沿用 `model`/`attachments` 先例）· 交互（子 agent 权限请求走同一条 `canUseTool` 桥，已通，至多加「来自子 agent」标记）· **显示（真正的工作量只在这）**——Task 工具行下如何挂子 agent 的文本/思考/工具、折叠还是展开、跑完收起还是留着。已有基础：`toolCard.ts` 已在渲染 Task 行。**开工先跑一次实测确认消息形状**（下述三处不打包票的第 1 条）。 | ① 实测确认真实子 agent 消息流形状后方可定 UI；② 主 agent 工具行不再混入子 agent 的调用；③ 协议为纯可选加法、旧版本忽略新字段；④ 事件量增幅可接受（16ms 批处理队列兜底后实测）；⑤ 三绿 | **已知限制（非阻塞，须显式登记）**：resume 重放读 Claude JSONL，`HistoryBlock` **无子 agent 归属概念**——当场看是嵌套的、关掉重开变平铺；与 D20 问答卡是**同一个协议缺口**，要根治须扩历史协议（**C-17**，本就后置） | 1.5d |
| **T-35** | Host stderr 进 UI（2026-08-05 立项，三条能力缺失之三） | **数据已在 Host 日志**：`src/agent-host/claudeRuntime.ts:677` 已把 CLI stderr 逐行记为 `[cli-stderr]`。本任务开一条事件推给前端 + **脱敏**（凭证/路径/token 形态），落在诊断向的可见位置（不进正文时间线，避免噪音淹没对话）。诊断向能力：平时用不到，出事时省很多时间。 | ① 造一次 CLI 失败能在 UI 看到首条 stderr；② 脱敏规则有单测（至少覆盖 `ANTHROPIC_*`、绝对路径、疑似 token 串）；③ 正常回合零噪音；④ 三绿。**✅ 2026-08-05 代码落地 `5a99ee1` + Codex 复核修复 `e602096`+`d3635d3`+`8bf4829`（终验两轮）**（as-built：新事件 `session.stderr` 纯可选加法（老端忽略未知类型，中枢盖章 seq/timestamp）；`stderrRedaction.ts` **摧毁式脱敏**——两轮复核加固后含 provider 裸 key 形态（sk-*≥16·sk-proj·sk_live/test·gh?_·AIza·AKIA）/泛化敏感赋值规则（`*_API_KEY`·`*_TOKEN`·`api_key`·`password` 等，JSON 引号键·含空格引号值·转义引号三形态**整值消费**）/大小写不敏感 `Bearer`·`Basic`（base64 可逆故整段销毁）/URL authority userinfo/x-api-key；**验收②「绝对路径」的执行裁定 = 用户目录前缀折叠 `~` 而非全路径抹除**（全抹则诊断价值归零；覆盖 POSIX·macOS·`C:\`·`C:/`·UNC·`\\wsl.localhost`·`/mnt/c` 变体，盘符规则前置防 POSIX 规则半吞；**Windows 系三族双档规则容空格/撇号用户名**（40 字符界+后随分隔符锚），POSIX 有意保守——放宽会贪吃两路径间散文，取舍注释入码）；单行 2000 字符钳制 + **每回合 50 行限流**（第 50 行为 capped 标记，Host 日志仍全量）；渲染端走 `sessionRuntimeFacts` 邻接 store（红线零改动）：环形 20 行 + running total + **载体会话数上限 12**（驱逐只剥 stderr 字段，**驱逐序 = stderr 首达序**——终验第二轮指出键插入序实为 entry 创建序，新载体首达时移键至图尾修正）；context surface「Host stderr」组序号从累计值起算（驱逐可见）、空组即隐；ChatComposer 超时提示指向更正。**GUI 点验待用户（0-septendecies）**） | 无（与协议/ACP 均无关） | 0.5d |

> **T-16 as-built（2026-08-05，`fd57ebf`）**：**实际强制覆盖是四处不是两处**——07-28 记录的 `App.tsx` 短路与 `Root.tsx` 水合后强写之外，还有 `App/useAppKeyboardShortcuts.ts`（主 tab 快捷键让位判断）与 `App/hooks/useSettingsState.ts`（`forceSettingsModal`）两处同形 `SKIP_ONBOARDING_GATE || …` OR，均为 07-28 之后新增，本任务一并拆除。
> **两处执行裁定（待用户追认）**：① **store 默认值 `false` → `true`**——覆盖期 `Root.tsx` 每次启动强写 `true`，既有 profile 全部已持久化 `true`，改默认只影响新 profile（否则新机首启落在旧标签页壳，而旧壳是回退不是产品）；② **新增同步镜像** `stores/settings/shellPreferenceMirror.ts`——`electronStorage` 是异步 IPC，首帧按默认值画壳、水合后再改写，开关关掉后每次启动会闪一帧新壳；改用 localStorage 同步镜像消除（范式同 `stores/shellLayout.ts` 的既有理由），权威仍在 settings，水合后回写镜像使外部改 settings.json 一次启动内收敛。
> **`SKIP_ONBOARDING_GATE` 维持 `true`**、不作为达成手段（口径不变）。防回归：`App/__tests__/shellSwitchStatic.test.ts`（TS AST 静态不变量：无文件用 dev flag 决定壳 / 无处代用户写偏好 / App·Root·store 三处形态钉死）+ `stores/settings/__tests__/shellPreferenceMirror.test.ts`。**验收①②③ 属 GUI 面，待用户点验**（清单 0-sexdecies）。
