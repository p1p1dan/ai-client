...
| **T-33** | 网络重试横幅（2026-08-05 立项，源自 2026-08-04 ACP 调研的三条能力缺失之一） | **数据已全通，只差 UI**：`stores/chatSessions.ts:85` 的 `retry?: SessionRetryInfo` 已由 Host 的 transport-retry 回合写入，且在**无 retry 载荷的 `session.status` 上会被清回 undefined**（该清除语义已存在，勿重写）。本任务只做呈现：回合区顶部一条非致命横幅，表达「不是卡死了，正在重试」+ 第几次/下次间隔（取 `SessionRetryInfo` 现有字段，缺字段即不渲染该段，A06）。**UI 逻辑下沉纯函数**（`deriveRetryBanner`），`.tsx` 零覆盖是本仓既有约束。 | ① 真实重试回合出现横幅、重试结束自动消失；② 无 retry 时零渲染（不留空盒）；③ 纯函数覆盖 SessionRetryInfo 各字段缺失组合；④ 三绿 | 无（数据已在红线 store，**只读不改**） | 0.5d |
...
| **T-35** | Host stderr 进 UI（2026-08-05 立项，三条能力缺失之三） | **数据已在 Host 日志**：`src/agent-host/claudeRuntime.ts:677` 已把 CLI stderr 逐行记为 `[cli-stderr]`。本任务开一条事件推给前端 + **脱敏**（凭证/路径/token 形态），落在诊断向的可见位置（不进正文时间线，避免噪音淹没对话）。诊断向能力：平时用不到，出事时省很多时间。 | ① 造一次 CLI 失败能在 UI 看到首条 stderr；② 脱敏规则有单测（至少覆盖 `ANTHROPIC_*`、绝对路径、疑似 token 串）；③ 正常回合零噪音；④ 三绿 | 无（与协议/ACP 均无关） | 0.5d |
