...
| **T-32** | 右栏骨架回归 A08（**D27**，2026-08-05 立项） | **开工前置 = 结 open-q #28（两处内部张力），未答不得动代码**。回归面（对照 [T-12~T-15 规格 §7](2026-08-04-t12-15-surface-spec.md)「被取代」十一项）：① `ContextPanel` 恢复 **tab 条**；② `ContextPanelRail` 改 **仅 panel 收起时渲染**（联动收展；展开时右缘无图标，L0 阈值因此不含 44px）；③ `MainHeader` 恢复**贯通中右**（与 T-23 刚落的单行 h-9 三顶条齐平口径**直接冲突，须一并重定**）；④ 恢复 **L0/L1/L2 画幅降级梯**（1580 = 280+400+520+380 / 1244 = 280+400+520+44）与 `manualPanel`/`manualChat` 会话内手动覆盖（关文件清零）；⑤ editor 归属按 #28 裁定（回中列则 `EditorSurfaceView` 搬家 + `editorRatio` + files tab 降为纯文件树）。**三项豁免不做**（D27）：terminal 不回 BottomDock（维持 surface，`BottomDock.tsx` 不复活）· context 不加 Quick notes/Todo/Plans · git 不加 branch/pr/sync/stash。**沿用不变**：宽度记忆 380–1400 · git-only dot · 44px Rail · 250ms 过渡（A08 与现状本就一致）。 | ① #28 两项均有书面裁定后方可开工；② 回归后 A08 的 tab 切换 / 收展 / 降级梯 / 手动覆盖四类状态迁移与 A08 §「状态机」表逐行吻合；③ 布局与 surface 选择逻辑**仍下沉纯函数**（不得回退成组件内联，`shellLayoutModel.ts` 现有断言面只增不减）；④ keep-alive 契约不回归（terminal 跑 `sleep 30` 切 tab 再切回，进程仍在、输出连续）；⑤ 三绿 + GUI 点验 | **open-q #28**（阻塞）· D27 · T-22 as-built · T-23（顶栏口径冲突） | 待估（#28 ① 决定量级：editor 回中列则 3d 级，不回则 1d 级） |
