...
| **T-30** | 观感打磨：分域字体落地 + 审计批次 1+2（**D25/D26**，2026-07-30 立项） | 依据 [`polish-audit-20260730.md`](../design/polish-audit-20260730.md)（34 条清单）：**批次 1 快赢 12 项**（删 USER 角标（回归基线）/user 气泡回归 A07/修 ModelSelect 默认值不落盘致 meta 丢 model 名的真 bug/`--tool-arg` 62%→78%/段头回归/接上 letter-spacing 梯度等，零基线修订）；**批次 2**（D25 分域字体：`--font-sans` 比例 + `--font-mono` 专供代码/路径/分支，含 design-system.md 字体节改写与两基线标注修订；D26 四条 A07 修订实施：失败行分级红/失败自动展开簇规则/侧栏 chip 放宽/气泡满宽；错误红字符 -86% 目标；排版层级 weight×tracking×color 三件套接通）；**与 T-23 交叠三项（MainHeader 死按钮/假环）不碰仍归 T-23** | ① 审计批次 1+2 清单逐项闭环（每项在审计文档标 ✅/偏离说明）；② 分域字体后中英混排/侧栏截断实测改善（对照截图入库）；③ A07/phase0a/design-system 三文档与代码同步（账实一致）；④ 纯函数单测；⑤ 三绿 + 用户 GUI 验收「协调感」。**✅ 2026-07-30 批次 1 代码落地 `3dcd2dc`**（6 文件 +95/-40；12 项快赢中 10 项落地——P-08 删 USER 角标/P-09 user 气泡回归 A07/P-11 NoticeMessage/P-06 失败框收敛/P-17 间距/P-05 destructive 降 70% color-mix/`--tool-arg` 62%→78%/P-14 模型名兜底/QuestionCard+LeftNav 字距梯度；**P-19/P-22 让给 T-23**，口径不变不重复登记；三绿：typecheck 干净/lint 672 文件 0 错误(外来 a08 html 29 警告不计)/vitest 71 文件 1097 例，与 T-19 同批验证、批1 本身无新增测试）。**批次 2 尚未开工**——施工依据已交付：deep-reasoner 产出 637 行字体规格（scratchpad `d25-font-design.md`）+ 编排者三条临时裁定（§2.3 细化点取舍/阅读栏 45rem 方案/待建 `a09-font-domain-baseline.html`），**均待用户 GUI 点验确认或否决**，详见 plantree implementation-status 与主线台账 2026-07-30 D25 规格交付行。 | D25/D26 ✅ · T-05/T-28 ✅ | 批1 ≈1d + 批2 ≈3d |
