...
## 9. 施工后追记：被推翻的设计与自引入的回归（2026-08-05 首轮点验后）

> 规格正文按 append-only 不改写；本节记录**实测推翻了什么**，供后续同类改造直接绕开。
> 八条缺陷（m1~m8）中**四条是本任务自己引入的**，都属于「代码评审看不出、只在截图里显形」那一类。

### 9.1 被推翻的原设计（规格错在哪）

| 原设计（§5） | 实测结果 | 现行做法 |
|---|---|---|
| 窄行时**让 chat 吸收缺口**、编辑器保底 | 800px 行给出 280px chat、Composer 65px 逐字换行——「读不了的窄条比没有更糟」 | **chat 有 400 硬下限**，放不下就不渲染（让位是降级的职责，不是压扁） |
| L0/L1/L2 **三档阈值常量**，侧栏在阶梯之外 | ① 280–500px 的侧栏岿然不动而 chat 被压碎；② L1 阈值把 44px rail **重复计入**（rail 是被测量行的兄弟节点） | `resolveShellChrome` 按实际预算**逐级试探**：sidebar → panel → chat。没有阈值常量就没有算术双重计入的空间 |
| 面板宽度只受 `availableWidth` 上限 | 面板可把整行吃光 | 新增 `maxPanelWidth`，面板**反向受内容下限约束** |
| 按 surface 的 `defaultWidthFraction` + per-surface 拖拽记忆（A08 v4「per-tab 宽度记忆」） | 每点一次 tab 面板就改宽；A08 **定稿**本身给三 tab 一个 380 默认（a08:1550），v4 的 per-tab 记忆与定稿矛盾 | 单一 `panelWidth`；整套按 surface 的宽度已删除并加静态 pin |

### 9.2 自引入的回归（改 A 打破 B）

1. **m6 门控 `EditorColumn` → m7 死循环**。`switchWorktree` **会清空 tabs**，而它的 effect 住在被门控的组件里：无 tab → 不挂载 → 点文件 → 首次挂载 → effect 清空 → 又卸载，文件永远出不来。
   **教训**：**清空型 effect 绝不能放在条件挂载的组件里**。「每工作区 tab 隔离」是**壳的工作区**属性，与编辑器开没开无关——放置位置从一开始就选错了，m6 只是让它显形。
2. **m5 统一收紧 `availableWidth` → 提升态盖不满**。内容下限预留对**停靠列**是对的，对**覆盖层**是错的。同一个量被两种形态共用即埋雷；已拆为 `availableWidth` / `maxDockedWidth` 两个入参。
3. **空 `flex-1` 占位盒**（m6-a）。组件返回 null 但外层盒子照常渲染，白占中列一半；因为空盒无背景，**肉眼与代码评审都看不出来**，只有量 Composer 宽度才发现。
4. **`bg-card/30` 从停靠列带进覆盖层**（m3）。同一个视觉 token 换了角色就失效——半透明的覆盖层等于没有覆盖。

### 9.3 三条通用坑（本仓已反复中招）

- **flex item 默认 `min-width: auto`**：不给 `min-w-0` 就拒绝收缩到内容以下，于是整行溢出去挤兄弟，而不是自己截断（m8 的 This PC 换行）。
- **持久化的顺序/尺寸会与代码里的默认值漂移**：`railOrder` 存了 T-32 前的顺序，于是 tab 条显示旧序、数字键按新序（m4）。**修法是让消费方吃同一份渲染用的数据**，而不是两边各自对齐一次。
- **ResizeObserver 测量滞后一帧**，在 250ms 宽度动画期间**全程**滞后；固定 px + `shrink-0` 的组合会在这段窗口里溢出（m3 的叠加）。改用可收缩的 `flex-basis` 才能优雅降级。
