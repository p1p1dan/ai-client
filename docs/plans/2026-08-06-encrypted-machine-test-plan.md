...
### 步骤 4b — 08-07 测试机五问题回归（`9a6cc01`，本轮新增）

> 这四项是 08-07 测试机反馈的修复，**只有 test.2 及以后的包才带**。
> 先在 About 里确认版本，再测这一节——在 test.1 上测等于测了个寂寞。
...
版本号**已抬到 `0.4.0-test.2`**（2026-08-08 由 test.1 再抬一档——08-07 测试机跑的就是 test.1，带五问题修复的新包必须与它区分得开，否则 About 核对这道保险失效），产物名与 About 里都能一眼分辨。
选带 `-test` 的预发布号有两层用意：① 与将来真正的 0.4.0 正式版不撞号；
② `allowPrerelease` 全仓为 false，**即使这个包被误发布，稳定通道用户也不会收到**——
这是压在 §0 三层保险之上的第四层。

**到手第一件事：About 里确认版本是 `0.4.0-test.2`。** 是 `test.1` 说明是 08-07 那个未修复的旧包，别往下测。
