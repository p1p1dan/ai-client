...
### 0.0 用户现场经验推翻的一条（2026-08-06，最高权威）

> 用户原话：「在加密设备上，我通过 npm 安装的 codex 可以正常使用和读取文件。」

编排者曾据源码结构推断「Codex 在 TSD 加密机上必读到密文」（理由：`codex.js` 仅 7,236 字节的启动器，
真正读写文件的是原生 `codex.exe`，若白名单按进程名则不命中）。**该推论与现场事实不符，作废。**

**保留下来的有效结论**：Claude 与 Codex 的进程结构确实不同（Claude 的 `cli.js` 是 20MB 纯 JS 跑在 node 内，
Codex 是启动器 + 原生二进制）[实测]——但**这个结构差异并未导致加密机上不可用**，
说明 TSD 的白名单机制比「按进程名」的转述更宽（具体机制不必深究）。

**对本裁定的影响**：T-11⑥ 的价值不变（仍要验随包 node.exe），但**不再存在「Codex 根本不可用」这个风险项**。
