...
**用户 2026-08-06 现场经验（推翻编排者的一处推论，权威）**：

> 「在加密设备上，我通过 npm 安装的 codex 可以正常使用和读取文件。」

编排者曾据「`codex.js` 只是启动器、真正读写的是原生 `codex.exe`」推断
**Codex 在 TSD 加密机上会读到密文**——**该推论与现场事实不符，作废**。
用户已在加密设备上实测：**npm 安装的 codex 读文件正常**。
→ TSD 的白名单机制不是「只认 node.exe 这一个进程名」那么简单（具体机制未知，也不必知道）。
**结论：Codex 在加密机上可用这件事已有现场证据，不构成 S2/S3 的阻塞。**

保留的有效部分：**「npm 安装的（走 node 入口的）」与「打包好的独立分发版」不是一回事**——
用户的原始修正正是这一条，仍然成立，仍是切片 2 的约束。

**开发态的实测数据不受影响**（探针本来就走 node 入口），**剩余风险在打包态**：
