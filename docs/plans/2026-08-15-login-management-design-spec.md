...
> **状态**：rev.1 待用户拍板 §8 四点；S0 spike 结论可能触发 rev.2。
...
**幂等实证（S0-E1，第一阻断项）**：白名单测试邮箱执行——① 首登记 `K1` 指纹（SHA-256，不留明文）；
② 二次验证码重跑记 `K2`，断言逐字段相等；③ 大小写/首尾空格变体（`User@JCDZ.CC` / ` user@jcdz.cc `）；
④ 两枚验证码**并发** verify，断言收敛同一 user/key；⑤ 用 `K1` 打 `/api/auth/login` 证旧 key 未被轮换；
⑥ 服务端侧核对 user/cch-user/active-key 计数均 =1。退路：直接查 onboard handler 的 upsert 逻辑一句话定论。

**证伪时的 fallback（S7，条件片）**：加统一端点 `POST /api/onboarding/verify-and-login`
（验证 OTP → 已有员工回既有 key / 不存在则走注册创建），响应与 `verify-and-register` 同构。
**否**客户端按本地 `registered` 选端点（本地状态不可信 + 验证码可能被首调烧掉——B 轨裁定）。
错误码增补 `ACCOUNT_DISABLED`（不可重试）/`KEY_REVOKED`（进重登）；`KEY_NOT_READY` 沿用（可重试不判死）。
...
- **双写过渡（A 轨裁定胜出）**：S1~S5 期间登录同时写 vault + 旧文件（=旧写手继续跑），任意时刻 flag off
  即回今天行为，**回退分支必须退到安全态**；S6 停双写并中和。B 轨「禁反向导出」保留为中和后不变量 I7。
- **中和（S6，外科式）**：`~/.claude/settings.json` 只删 app 三键（hooks/statusLine/用户键逐字节保留，
  先 `.bak`）；`~/.codex/auth.json` 只删 `OPENAI_API_KEY` 字段**保留文件**；config.toml 只摘 `[model_providers.jyw]`
  与指向它的 `model_provider`。**顺带修既有 bug**：`removeCodexConfig` 的 `rmSync` 整删（会毁用户自有配置），
  登出与中和共用新的外科实现。`.bak` 按 key 指纹识别再清（B 轨 R10）。
- 中和后 flag off = 需一次重登（旧路径会重写旧文件），诚实登记；vault 不受 flag 影响，翻回 on 即时恢复。
...
| 片 | 内容 | 关键断言（摘） |
|---|---|---|
| **S0** | 六 spike，不动产品代码：E1 幂等（§5）· E2 `$CLAUDE_CONFIG_DIR/.claude.json` 信任标志实测 · E3 SDK `settingSources:[]` + 无 `~/.claude` 全会话 · E4 app-server（非 exec）缺 env_key 报错帧 · E5 cch 401/403 可区分性（业务 403 vs auth 拒）· E6 safeStorage 三平台矩阵 | trace + fixture 落库；E1 定 S7 存废，E2/E3 定 §3.A 主干成立性 |
...
| **S6** | 收编 + 停双写 + 外科中和 + 修 rmSync bug + `.bak` 指纹清理 | marker 单次；hooks 数组中和前后逐字节同；auth.json 文件在、仅字段消失；无凭据机器 no-op |
| **S7**（条件） | E1 证伪才做：`verify-and-login` 对接 + 错误码 | 新机器登录拿回同一把 key，旧机器不掉线 |

**通用负控**（B 轨，每片附带）：造「key 进 IPC 返回」「key 写进生成 config」「空进程列表通过终止断言」
「provider 段塞错层级字段」等违规实现，断言测试变红——对齐本仓变异验证纪律。

## §8 需用户拍板（rev.1 → Go 的前置）

| # | 问题 | 推荐 |
|---|---|---|
| U1 | 存量机器旧凭据：收编导入（免重登）后，稳定一轮再外科清理？关键事实问：**有没有员工在 app 之外的系统终端直接跑 claude/codex**（清理会断这条路径） | 收编 + 延迟外科清理 |
| U2 | `SKIP_ONBOARDING_GATE` 收回节奏（收回后 dev 启动默认要登录，逃生舱 env 兜底）——影响全队日常 | S5 收回 + `AICLIENT_SKIP_AUTH_GATE=1` 逃生舱 |
| U3 | 幂等实证的落地方式：给测试邮箱跑八步 spike？onboard 服务由谁改（若证伪需加 `verify-and-login`） | 测试邮箱 + 你侧确认 handler 逻辑 |
| U4 | 换邮箱重登时旧账号本地历史（B 轨发现的越权面）：隔离不删/禁止换邮箱/自动删 | 绑 accountId 隔离 + 提供手动清除 |

设计者已默认（不上拍板，异议可改）：safeStorage 降级明文 0600 不阻断；卸载留存不动；上下文两键本轮不写；
终端 codex 共用托管 home（接受 app 强制 posture，逃生舱 `codex --config` 覆盖）。
...
R6 safeStorage Linux 退化（低）· R7 codex home 并发（低）· R8 portable 与安装版 userData 同异未证（低）。
**O1** onboard 幂等（E1）｜ **O2** app-server 无凭据错误帧（E4）｜ **O3** cch 401/403 语义细分（E5）｜
