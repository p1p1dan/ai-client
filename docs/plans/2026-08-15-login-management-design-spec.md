...
> E5 实证注记：actions 端点**只认 cookie**，有效 key 的裸 bearer 也 401——业务 401 是常态而非异常，
> 判失效唯一权威 = `/api/auth/login` 的 `401 KEY_INVALID`（两臂形状 fixture 见 e5 报告）。
...
**O1** ~~onboard 幂等~~（已关闭：代码级 CONFIRMED + **E1-lite 线上双轮实打**——变体邮箱回发同一 key 指纹，报告 e1-deployment-consistency.md）｜ ~~O2~~（已闭：E4——错误只在 turn 级异步 `error` 通知 + `turn/completed(failed)` 浮出（`Missing environment variable`），initialize/thread-start/turn-start RPC 全成功；registry 前置 `probeCredentials` 因此为唯一正门，帧 fixture 见 e4 报告）｜ ~~O3~~（已闭：E5 补测——真 cch 上 key 无效 = `401`+JSON+`errorCode:"KEY_INVALID"`，与 404(HTML)/307(重定向) 三维可区分；valid 臂已随 E1-lite 补齐：login 200/cookie 7 天 HttpOnly；**业务端点连有效 bearer 都 401（只认 cookie）**——业务 401 绝非失效信号，I6 判据升级为双臂实证；`/api/actions` 弃用 sunset 2026-12-31 已入 ideas）｜
