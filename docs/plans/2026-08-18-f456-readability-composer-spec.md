...
## §11 as-built 实录（2026-08-18 集成者回填，四片合并后）

- **四片 commit**：① `b1b8f6c`（3 文件 +91/−7；语义 token 表行同步由集成者按 §5.2 裁定补齐）·
  ② `1c5a797`（9 文件 +677/−99）· ③ `1516623`（6 文件 +608/−305）· ④ `19a65f7`（14 文件 +1015/−84）。
- **决策号**：按合入时点台账实况复取，本批落 **D50**（F2 批 D49 先行，§9.2 号位竞争警告命中）。
- **收官终跑**：④ 片执行，全量 vitest **239 文件 / 4724 例**两轮全绿（变异全还原后复跑）；四门逐门串行绿。
- **变异**：① 4 发 · ② 15 发 · ③ 5+2 发（补 M-16b/c）· ④ 9 发——零存活零跳过，全 md5 对账还原。
- **§4.6 否决权终态**：未撤销未折中；条件②当场证否（720→690px 朝最优行长靠拢）；条件①③连同
  ②发现的「散文→工具→散文容器挂/卸」机制证据移交 GUI 点验，若认定可见跳动由用户拍板降级方案。
- **规格内部矛盾实录**（施工中发现，均按「命题成立、取值修正」处置）：§1.6「:433-435 不改」实为必改；
  「leading-normal 9→6」实为 9→7；render 测试「改值 1 条」实为 4 条；§7.1 retry 后缀位置两读法取「位置不动」；
  §9.2 片③测试清单漏 `composerAgentPickerWiring.test.ts`（必红）；`[F4-9]` 渲染断言按 node 环境纪律改源文+行为两半；
  AST 工法实际 ~150 行（估 40~60）。
- **遗留**：§8.6 ② 后半（`< SEND_SILENCE_CEILING_MS/1000` 跨模块断言）未落；Q3 全局 UI 语言口径待用户拍板
  （不阻断）；GUI 点验 G-1~G-11 + ②③④ 新增项随下轮点验批。
