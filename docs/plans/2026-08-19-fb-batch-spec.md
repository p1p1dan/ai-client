...
| **Q15**（rev.2 as-built 新增，**待拍板**） | **FB3 的展开控件在「提问根本没被截断」时也常显，是否可接受？** 若不可接受，是接受一次几何测量（`scrollHeight > clientHeight`）来收敛显隐，还是换别的形态（如把展开做进 `title`/双击）？ | **先维持恒显**；不可接受时**倾向加测量**，但须先过 F10 回路复核 | 【实测 2026-08-23，G-4】样本提问 222 字符渲染 6 行，`clientHeight === scrollHeight === 146`（未截断），`Show more` 仍显示。<br>施工时**故意没做测量**：按「是否溢出」决定显隐要在气泡内读**元素几何**，而 F10 修掉的正是 `scroll position → clamp → height → scroll position` 这条边（§3.3）。<br>⚠️ **但两者并不同构**：F10 的环里，几何**反过来改变 clamp**；此处几何只决定**按钮显隐**，按钮是 clamp 容器的**兄弟**，不改其 `scrollHeight`/`clientHeight` ⇒ **不成环**。所以「加测量」在技术上是安全的，只是**没被规格授权**，且会给每个气泡挂一个观察者。<br>⇒ **是观感与成本的取舍，交用户定。** |
...
| **G-4 · 短提问下控件常显** | ⚠️ **问题已实测坐实，待用户裁定**。样本提问 222 字符渲染为 6 行，`clientHeight === scrollHeight === 146`（**根本没有截断**），而 `Show more` **仍然显示**。这正是片⑤ as-built 偏差 ③ 预告的观感问题 ⇒ **需用户看图后定去留**（见下方 Q15） |
