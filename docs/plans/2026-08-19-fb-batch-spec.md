...
✅ **【已实测 2026-08-25，片④ 开工前微基准】两条推测都成立，且解析侧比推测更极端。**
工法：`renderToStaticMarkup` + 真实 `<ChatMarkdown>`，100,188 B 语料（标题/散文/列表/表格/围栏代码混排，**每单元唯一**——重复语料会被内容去重污染分段侧数字），40 次 flush 模拟流式追加。

| 量 | 实测 |
|---|---|
| **不分段**（单个 `<ChatMarkdown>` 吃整个 closedPrefix，每次 flush 全量重解析） | **6379 ms / 40 次** |
| **分段**（只解析字符串变过的段，即 `React.memo` 命中后的真实工作量） | **165 ms / 289 段** |
| **比值** | **38.7×**（另一轮 51.2×，机器负载波动，量级一致） |
| **切分扫描器**（`advanceClosedPrefix` 全量重扫） | 全程 **11.7 ms**，**单次最坏 1.71 ms** |

⇒ ① 「分段渲染」不是优化偏好而是必需：不分段时单次全量重解析远超一帧；② 「扫描成本可忽略 ⇒ 全量重扫」成立 —— 推测写「亚毫秒~1ms」，实测 1.71ms，同量级，增量扫描器状态确实不值这点收益。
⚠️ 微基准是**一次性取证**，不留在测试套里（6.5s 单测会拖慢全量）；红/绿裁决仍归 `[FB1-6]`。
...
| **G-6** | 一个**多次交错**的回合（正文 → 工具 → 正文 → 工具 → 正文） | ⚠️ **判据已由用户订正（2026-08-25）：验的是「输出内容的条理、逻辑、可读性」——「该折叠的折叠，不该折叠的内容正常显示」，不是数边框盒。**<br>① **结构对不对**：工具/思考/授权（噪音）是否都在折叠壳内；正文是否**恒在壳外、按原顺序**、不被尾部 notice 连坐；② **读起来顺不顺**：交错回合从上到下是否读得出「说了什么 → 做了什么 → 又说了什么」的因果；③ 边框只在**它本身妨碍上面两条**时才成为问题。🚫 **判定不可接受时，施工方不得自行启用 C3** —— 走 F456 §4.6 三步上报，**由用户拍板**（见 §4.5 红框与 §12.1 **Q14**） | §4.5 / §12 Q14 |
...
| # | 问题 | 本规格的建议 | 为什么必须问 |
|---|---|---|---|
| **Q14**（rev.2 新增，**待拍板**） | **FB4-C1「每段一容器」的碎片化是否可接受？不可接受时是降级到 C3（只有单 answer 段时才挂容器），还是撤 D3-b？** | **先按 C1 施工**（它是 §4.5 的主推），等 GUI 出图后再定；施工方**不得**自行降级 | 这条降级权**明文保留给用户**，不在施工方手里：F456 规格 `:1047-1049`「由**用户拍板**是否降级……**施工方不得自行撤销 D3-b** —— 它是用户在知情（设计员已反对）情况下的拍板，只有用户能推翻自己的拍板」；同档 `:1928` 列为**禁止动作**；总台账 D50 行（`ledger.md:94`）记有「**实现方否决权保留：§4.6 三条件**」，且 `:1962` 的 as-built 表明**至今未行使、仍然开着**。<br>**判据 = G-6**（多次交错回合的截图，亮暗双主题各一张）。<br>⚠️ **判据经用户订正（2026-08-25）：重点不是碎片化，是「输出内容显示的条理、逻辑和可读性 —— 该折叠的折叠，不该折叠的内容正常显示」。**⇒ 先看**结构**（噪音是否都进了折叠壳、正文是否恒在壳外且保序）与**可读性**（交错回合是否读得出因果），边框只在妨碍这两条时才成为问题。这把 Q14 的重心从装饰移回结构 —— 而结构正是 FB4 本体要交付的东西。<br>**流程**：G-6 判定不可接受 ⇒ 走 F456 §4.6 三步上报（① 施工分支对照截图 ② 在 F456 规格追加 §4.6-a 实作否决记录 ③ 用户拍板降级方案），**不得**在本批内自行切 C3。 |
...
- **E-2（待用户拍板）**：**Q14** —— 判据 G-6，**看过 GUI 出图后再定**。⚠️ **2026-08-25 用户订正判据**：不是碎片化，是输出的**条理/逻辑/可读性**（该折叠的折叠、不该折叠的正常显示）。
...
### 片④ —— FB1-b + FB9（2026-08-25 落地，`f57387fd` / `65ee39be` / `e0a3c136`）

**门禁**：全量 vitest **245 文件 / 4962 例全绿** · `pnpm typecheck` 0 · biome 582 文件 0。
**变异**：本片 12 发（FB1-b 侧 M-04 · M-01w · M-05w · M-34 · M-GAP；FB9 侧 M-26~M-28 · M-32 · M-33 · M-PC · M-GATE）**全部实跑、12/12 咬红、零存活**。

#### 开工前微基准（§1.4 的两条【推测】实测收口）

工法：`renderToStaticMarkup` + 真实 `<ChatMarkdown>`，100,188 B 语料（标题/散文/列表/表格/围栏代码混排，**每单元唯一** —— 重复语料会被内容去重污染分段侧数字），40 次 flush。

| 量 | 推测 | 实测 |
|---|---|---|
| 不分段（单 `<ChatMarkdown>` 吃整个 closedPrefix） | 「每增长一次就整段重解析」 | **6379 ms / 40 次** |
| 分段（只解析字符串变过的段） | —— | **165 ms / 289 段** |
| 比值 | —— | **38.7×**（另一轮 51.2×） |
| 切分扫描器 | 「单次最坏 ≈ 亚毫秒~1ms」 | 全程 **11.7 ms**，单次最坏 **1.71 ms** |

⇒ 分段是**必需**不是优化；无状态全量重扫成立。取证脚本不入库（6.5s 单测拖慢全量）。

#### ⚠️ M-04 首轮存活 —— 本批第六发同型空转臂

`[FB1-6]`（片① 落的确定性工作量断言）断的是**纯函数**产出的工作量：`advanceClosedPrefix` 返回的段。M-04 不动纯函数，它把段 **join 回一坨**喂给单个 `<ChatMarkdown>` —— 输出逐字相同、`[FB1-6]` 照绿、解析量回到 O(n²)。
⇒ 补 `[FB1-7]`（`TurnTextItem` 内必须 `split.segments.map(` 一段一个元素，且不得 `segments.join(`）后咬住。
**形状与前五发完全一致：断言在错的那一层。** 记账见 §15 末尾的空转臂总表。

#### FB9 的三处实测收口

| 项 | 结论 |
|---|---|
| **字体（E-1 复现）** | 仓内最小 Vite lib 探针 + **真实安装版本**：557,722 B 单 bundle · katex 引用 21 处 · **字体文件 0 · `data:font` 0 · `@font-face` 0** ⇒ 字体红线成立 |
| **§8.2 ④ 的【推测】**（rehype-katex 是否走 raw 节点路径） | ✅ **当场判定**：渲染输出是 hast 树转 React 元素，无 `dangerouslySetInnerHTML`、无 raw。不必推迟到施工期，一条 `render('$$e=mc^2$$')` 就够 |
| **安全（运行时攻击七臂）** | 全部无 `<a href>` / `<img>` / 带 `src` 的元素 / `<script>` / 事件属性；`\href` 与 `\includegraphics` 降级为 KaTeX 红色错误标记（`#cc0000`）。**M-28（`trust: true`）真的让 `[FB9-9]` 咬红** —— 打开后确实产出可点链接，这组断言是真守卫 |

⚠️ **一处需要写明的判定**：危险 URL **确实会出现在 `<annotation encoding="application/x-tex">` 里**（KaTeX 把原始 TeX 源放在那），但那是 MathML 语义注解节点 —— **不渲染、不可点、不发请求**。`[FB9-9]` 因此**显式剥离 annotation 再查**，而不是笼统禁字符串：后者会因一次无害回显而红，且教不会下一个人任何东西。

#### G-10（2026-08-25 真机）：抓到一个真缺陷，已修

初测：三条公式全部进 MathML（3 个 `<math>`、0 个 `.katex-html`），行内 `$` **零误伤**（「价格从 $5 涨到 $10，执行 echo $PATH」原样保留）。**但分数线几乎不可见、分子分母重叠。**

CDP 逐步替换定位：

| `font-family` | 分数线 | 矩阵括号 |
|---|---|---|
| `math`（Chromium UA 默认） | ❌ 坏 | ❌ 不拉伸 |
| `'Noto Sans Math'`（显式点名） | ✅ **完全正确** | ❌ 不拉伸 |

⇒ **根因不是 MathML 本身**，是 Chromium 的 `math` 泛型族在 Linux 上经 fontconfig 解析，而 fontconfig 通常**没有 `math` 别名** —— 落到一个没有 OpenType `MATH` 表的字体上，分数线/根号/可拉伸括号都画不出来。
**修法**：`globals.css` 新增 `--font-math` **系统回落链** + 一条 `math { font-family }` 规则。**不破字体红线** —— 红线原话是「naming a **non-system** family here is a blank cheque」，而这里每个名字都是 OS 自带的面，与 `--font-sans` 同范式；零 `@font-face`、零字体资产，终结于 `math` 泛型族。

**残留（用户已拍板接受 ⇒ 台账 D57）**：矩阵 `pmatrix` 的括号不随高度拉伸，CSS 修不了。⇒ 不启动红线偏离、(b) 路不走，形态记 [inbox 另立票](../plantree/ideas/inbox.md)。

#### 与 rev.2-b 的偏差（四条）

| # | 规格 | 实际 | 原因 |
|---|---|---|---|
| ① | `[FB9-5]` 落 `packaging-config.test.mjs` **或**构建产物扫描 | **两者都做，但分工不同**：新增 `scripts/assert-no-webfonts.mjs` 扫 `out/renderer`，接入 `dist:prereq`；`packaging-config.test.mjs` 里的断言钉的是**那个门存在且接线正确** | 产物扫描需要产物存在。做成条件跳过的单测就是空壳 —— **看不到产物就放行的门比没有门更糟**；做成流水线步骤则在真正出包的地方跑 |
| ② | §9.1 无 `[FB1-7]` | **施工期新增** | M-04 的唯一有效发射半边，见上 |
| ③ | §1.4 只说「段容器补间距，建议 `space-y-3.5`」 | **导出 `chatMarkdownSegmentGapClass()`**，并断言其数值**读自 `BLOCK_GAP`** 而非重复字面量 | 「同档同值」若靠两处写同一个数维持，就是下一次漂移的起点 |
| ④ | §8.4 未提字体族解析问题 | **新增 `--font-math` token** | G-10 实测发现，见上。规格假设 `output:'mathml'` 就够了，实测在 Linux 上不够 |

#### 未做的点验

**G-2**（≥50KB 回答的帧率观感 —— 微基准已覆盖其定量部分）· **G-11**（打包产物里的公式渲染 —— 本机跑不动全量构建，`assert-no-webfonts` 门已在 `dist:prereq` 里等着）。暗色主题图未出。

### 片③ —— FB4 + FB6（2026-08-25 落地，`871e8040` / `a13046ec` / `4a1c8402`）

**门禁**：全量 vitest **245 文件 / 4946 例全绿** · `pnpm typecheck` 0 · biome 161 文件 0。
**变异**：本片分配 12 臂（M-11~M-18 · M-12b · M-18b · M-31 · M-35）**全部实跑、12/12 咬红、零存活**。
⚠️ **M-31 首轮存活** —— `[FB6-5]` 原本只查字面量 `TurnCopyButton`，抓不到「一个**会渲染**它的组件」（`TurnMetaTail`）被塞进触发器；改成**子树白名单**（触发器子元素只能是 `ChevronDown`）后咬住。**第五发结构性空转臂**，与 rev.2 记账的四发同族。

**FB6 的三步证据链（§5.6-a）如实走完**：① 扩定位器支持 `memo(function …)` 与「取 `return` 语句的根 JSX」（原 `firstJsxIn` 会抓到 `renderItem` 里的 `<TurnItemView>`），补 `[FB6-0]` 脚手架对旧码**跑绿** → ② 移结构后**变红** → ③ 换成永久的 `[FB6-1]`，脚手架删除。**此前全仓没有任何断言钉住 `ChatTurn` 的子元素顺序**，这条网是本批最重要的新钉子。

#### GUI 点验抓到两个断言抓不到的缺陷

| # | 缺陷 | 为什么断言没抓到 |
|---|---|---|
| ① | **meta 行的折叠钮是死控件** —— 触发器从 `CollapsibleTrigger` 搬到 meta 行后 Base UI 不再代接点击，我没补 `onClick` | `aria-expanded` / `aria-controls` / `disabled` / 子树白名单**四条全绿**，绿在一个按什么都不发生的按钮上。补 `[FB6-8]`（后随折叠退役一并退役，但教训记在退役注里） |
| ② | **§4.6 裁定的 E1 方案根本不工作** —— 折叠展开后收不回去 | 见下 |

**E1 失效的根因（读 Base UI 源码坐实，非猜测）**：`useCollapsibleRoot` 只在两处清 panel 的 `mounted` —— `:82` 仅由 `CollapsibleTrigger` 的 `handleTrigger` 调用；`:91` 是受控分支但要求 `!keepMounted`。本片的组合是「**受控 `open` + `keepMounted` + 触发器在 Root 之外**」，**两条都不满足**，`setMounted(false)` 永不执行 ⇒ `aria-expanded` 翻转、`data-ending-style` 落上，但 `hidden` 永远加不上。
⚠️ **回退到片③ 之前的代码实测折叠正常**，确认是本片引入、非既存缺陷。
⇒ §4.6 选 E1 的论证核了 panel id 竞争，**没核「谁来调 `setMounted`」** —— 这是规格的取证缺口。改用 §4.6 已列的 **E2**（普通 `<div hidden>`），R4「keepMounted」红线**反而满足得更直接**：子树从不卸载。`turnProcessPanelClass()` 及其整批断言随之退役（每一条类都是为了把 Base UI 从「测量+裁剪高度」的路上拉下来，而 `hidden` 面板不测量）。

#### 用户拍板（2026-08-25，看图后当场决定 ⇒ 台账 **D56**）

| # | 拍板 | 处置 |
|---|---|---|
| ① | **Q14 = C1 可接受**，不降级 C3、不撤 D3-b | **D50 的实现方否决权本轮未行使、仍然开着**。⚠️ 用户当场**订正判据**：G-6 验的是「条理、逻辑、可读性 —— 该折叠的折叠，不该折叠的正常显示」，**不是数边框盒**（§10 G-6 与 §12.1 Q14 已同步订正） |
| ② | **回合级折叠退役**，meta 行只保留信息 | 理由是每个 tool 行自己就能展开，FB4 之后回合级折叠已无多少可藏。连带退役五个纯函数（`defaultTurnProcessOpen` / `TurnProcessOpenInput` / `collapsedLeavesNothing` / `hasUnresolvedPermission` / `turnHasFailure`）与 `[FB6-6]`/`[FB6-7]`/`[FB6-8]`/旧`[FB6-4]`/`[FB6-9]`/F5/F10/`[FB4-5]`。**授权红线由「条件」升为「结构」** —— process 段无条件渲染、授权卡根本藏不了，新 `[FB6-4]` 用三条否定钉死（无可见性绑定 / 不被条件渲染 / 不得再引入 `Collapsible`） |
| ③ | **工具行默认折叠** | **推翻既有 sign-off ②「failures auto-expand」**：`ToolRows` 改 `defaultOpen={view.defaultOpen ?? false}`，T-34 的 LIVE subagent 面板成为唯一默认展开者 |

#### 与 rev.2-b 的偏差（五条）

| # | 规格 | 实际 | 原因 |
|---|---|---|---|
| ① | §4.6 取 **E1**（受控 Collapsible + 外置触发器） | ❌ **改 E2** | E1 实测不工作，见上 |
| ② | §5.3 取 **G1′**（行尾独立 chevron 触发器） | ❌ **触发器整体退役** | D56 ② |
| ③ | §4.4「空 process 时折叠头仍渲染，降级为非触发器」 | ⚠️ **整条 moot** | 没有触发器了，meta 行恒为纯信息行 |
| ④ | §9.1 `[FB6-2]` 只写「底部行仍带 truncate」 | **补源码半边** | 类装配上查不到 —— `truncate` 在 `TurnMetaTail` / `TurnHeadContent` 的 span 上，不在 `turnMetaRowClass()` 里 |
| ⑤ | §9.1 无 `[FB6-8]` | **施工期新增又退役** | 死控件那一发；退役注里保留教训 |

#### 未做的点验

**G-5**（贴底跟随态展开）· **G-7**（band 下第一元素是带边框容器）· **G-12**（chevron 朝向 —— 随触发器退役而 moot）· **G-13**（底部三行堆叠 —— 合并成一行后 moot）· **G-14**（PendingTurnHead 相邻）· **G-15**（多段一起收起 —— 随折叠退役而 moot）· **G-16**（键盘可达 —— 触发器退役后只剩 copy，需重定范围）。暗色主题图未出。
...
## §16 空转臂总表（本批最该被下一个人读到的东西）

rev.2 的双轨评审记账过「rev.1 有四发结构性空转臂」。**施工期又实测出六发**，形状完全一致，而且每一发都是**断言全绿、变异存活**才暴露的。

| # | 变异 | 本该咬它的断言 | 为什么咬不到 | 补的半边 |
|---|---|---|---|---|
| 1 | **M-24**（片①）`formatThoughtRow` 复制一份分钟换算 | `[FB8-2]`「时长片段逐字等于 `formatWorkedForDuration(同输入)`」 | **忠实复制的换算输出完全相同** —— 行为断言分不清「复用」与「抄一份」 | 源码半边：函数体内必须调用共享格式化器，且全文件 `% 60` 只出现一次 |
| 2 | **M-06 / M-10**（片⑤，rev.2 已预判） | `[FB2-1]` / `[FB3-2]` | 落点 `chatTimelineLayout.test.ts` **读不到** `ChatCodeBlock.tsx` / `MessageTimeline.tsx` 的源码 | 落点改 `messageTimelineWiring.test.ts` 同族源文扫描 |
| 3 | **M-36**（片②）徽记颜色单臂硬编码 `text-tool-arg` | `[FB7-7]`「不含 `bg-` / `border` / 图标组件名」 | `text-tool-arg` **三样都不含** —— 老断言查的是「有没有装饰」，不是「颜色对不对」 | 双臂形状约束：要么两臂都无颜色类，要么两臂取值不同 |
| 4 | **M-31**（片③）把 copy 塞进折叠触发器 | `[FB6-5]`「触发器子树不含 `TurnCopyButton`」 | 塞进去的是 `TurnMetaTail` —— **一个会渲染它的组件**。黑名单只认它知道的那个名字 | 改**子树白名单**：触发器的子元素只能是 `ChevronDown` |
| 5 | **（GUI 抓的，非变异）**片③ meta 行折叠钮无 `onClick` | `aria-expanded` / `aria-controls` / `disabled` / 子树白名单**四条** | 四条全是**属性**断言，一个属性齐全的死控件与活控件长得一模一样 | `[FB6-8]`：触发器必须绑到状态切换。**这一发是截图抓的，不是测试套** |
| 6 | **M-04**（片④）把段 join 回一坨喂给单个 `<ChatMarkdown>` | `[FB1-6]` 确定性工作量断言 | 它断的是**纯函数产出**的工作量；join 不动纯函数，**输出逐字相同**，解析量却回到 O(n²) | `[FB1-7]`：`segments.map(` 一段一个元素，禁 `segments.join(` |

### 三条可复用的判据

1. **断言必须与变异在同一层。** 第 1、6 发都是「模型层断言 + 消费层变异」。纯函数证明了它交出正确的东西，证明不了有人正确地用它。
2. **黑名单只能挡它知道的名字。** 第 3、4 发都是黑名单：一个不在名单上的等价物（`text-tool-arg` / `TurnMetaTail`）直接绕过。**能写白名单就写白名单。**
3. **属性齐全 ≠ 活着。** 第 5 发是本批唯一靠截图而非测试抓到的，且它在四条断言的注视下存活。凡是「控件」，必须有一条断言钉住**它被接到了状态**，而不只是它长得像个控件。

### 一条连带纪律：负向源码断言必须剥注释

本批**撞了三次**：`ChatCodeBlock.tsx` 头注为解释「为什么禁 hover-only」写了 `opacity-0`；`toolCard.ts` 头注为解释「为什么不重抄决议词」写了 `Denied`；`globals.css` 的字体红线原话里就有 `@font-face`。
⇒ **凡负向扫描，一律先剥注释。** 一条会因为解释自己而失败的规则，没人会长期遵守。
