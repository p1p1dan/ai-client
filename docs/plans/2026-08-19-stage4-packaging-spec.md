...
> **as-built（2026-08-21 回填，含一处偏离）**：本条在 P4 首版实现中**整条缺失**（外部复审第 7 条抓出），现已补为 `evaluateAppDirSize` + WARN-only 接线，永不计红。
> **偏离**：基线项没有采用规格写的「413 MB」。理由三条 —— ① 该数**无量纲**（413 MB 十进制与 413 MiB 差 20 MB，违反 §0.3 量纲纪律，不得直接硬编成字节常量）；② 它早于本批的 Electron/monaco 变动，已经过时；③ **Windows 根本没有前置测量**（本仓此前从未产出过 Windows 产物）。
> 改为由实测推导 `基线 = appDir 实测 − P`（CI run 32448401467）：linux-x64 `990,967,116 − 363,716,282 = 627,250,834`；win32-x64 `1,198,625,823 − 427,157,004 = 771,468,819`。
> **代价明说**：这样得到的上限等价于「当前 appDir 的两倍」，比规格锚点更松。鉴于本条自身定位就是「只挡整体失控且不计红灯」，接受该偏离并在此登记，不再私下消化。
