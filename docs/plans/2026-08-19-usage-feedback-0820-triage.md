...
| # | 议题 | 拍板 |
|---|---|---|
| Q1 | 工具 IN（命令）/OUT（结果）块是否也加 copy（安全边界 `turnCopy.ts:6-10` 改判） | 待 |
| Q2 | FB7 Ran/Allowed 双行：合并（L）/样式收窄（S）/维持 | 待 |
| Q3 | FB9 LaTeX 渲染：本批做（M+安全评审）/另立/不做 | 待 |
