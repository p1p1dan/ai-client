...
2026-09-13 落地注记（P6-1 / P6-4 / P6-5，同一天两步）：

1. **P6-1** 把默认切到 native，判读方向随之反转：只有精确的 `legacy` 走旧引擎。
2. 同日**用户决定提前执行 P6-5**，不等一个版本周期。旧引擎退役后这个开关没有可选项了，
   于是**连同 `src/shared/runtimeBackend.ts` 一起删除**——设这个变量现在完全没有效果。

所以本条的「保留一个版本周期」没有走完：回退窗口由用户主动放弃，回退方式改为
**装回上一个安装包**（[回退说明](../pi-only-rollout-rollback.md)）。这一步的代价是明写的：
出问题时不能靠一个环境变量切回去。用户在知道这一点的前提下选了立即退役。
...
**实际执行（2026-09-13）**：第 2、3 步都已走完，但第 3 步的「保留一个版本周期」由用户当日
决定放弃——默认切换与旧层退役同日完成，开关一并删除。见 D8 的落地注记。
...
3. `pi-coding-agent` 从 `package.json` 移除，`pi-agent-core` + `pi-ai` 成为仅有的两个 pi 系依赖。
   **2026-09-13 注记**：写这条时该包只是一个库；此后 H/18～H/20 让它又成了内嵌 Pi 终端与插件管理
   （H/19 的装/卸/列）的**执行体**，删掉它会连带下线这两项功能，也会让 P6-4 的回退开关没有回退对象。
   本条因此拆成两层：**实质**（我们的 agent 不再跑在别人的整包上）与**字面**（package.json 里不出现这个名字）。
   **用户 2026-09-13 决定按口径 A：保留为随包的可执行文件，放弃它的库角色。**
   同日执行 P6-5 后这条落到了终点：**应用代码里一处都不再 import 它**（允许名单已清空），
   终端与插件管理只通过**进程**用它的 `cli.js`。本条按**实质达成**签收——
   字面上 `package.json` 仍有这个名字，那是「随包可执行文件」这条口径的结果，不是遗留。
   三道守卫：`piCliIsBundledToolOnly.test.ts`（越界 import 当场点名，并反过来断言终端与
   插件管理仍在用它——保留它的唯一理由）、`nativeWorkerDependencyBoundary.test.ts`（静态导入图）、
   `nativeWorkerModuleLoads.test.ts`（真实 worker 进程实测零加载，另起一个进程真加载它作阳性对照）。
   见 [P6 施工计划](../plantree/plans/runtime-evolution/topics/p6-cutover.md)。
