...
1. 搬 PI-Desktop 的 `createProviderRetryStream`，把 **429 / 503 / 529 + 连接类错误**纳入可重试集合，尊重 `Retry-After`。
2. 现场只触发到 503、未触发 529/429，是因为可重试错误本来就难稳定复现；**补上重试层后按错误类型做单测**，不依赖现场撞运气。
3. **F7d「GPT 渠道耗时极长」很可能是同一根因**（SDK 默认阶梯在跑），补完重试层后一并复测；在此之前不要按「渠道兼容性」单独排查 `reasoning_effort` 之外的部分。
...
| F4 | 漏搬 `provider-retry.ts` | P4 遗留项 | 是 |
