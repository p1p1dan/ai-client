...
| D47 | 用户登录管理立项五点（用户拍板 2026-08-15 当场两轮问答；multi-agent 阶段 2 立项） | ① **范围 = 凭据权威收敛到 app**：现有「邮箱+验证码注册」雏形升级为正式登录态管理（登录/登出/凭据托管），内部员工工具、员工零感知 claude/codex 凭证；② **登录方式只用邮箱+验证码**，不建密码体系；③ **服务端最小改**：固定业务代码不动，onboard 服务可改，目标语义「登录 = 换取该员工既有 key」（客户端假定 `verify-and-register` 对已注册邮箱幂等回发同一 key——需实证，不成立则补最小登录接口）；④ **单账号**：一员工一账号，不做多账号切换；⑤ **凭据落盘改 app 私有托管 + 进程注入**：key 只存 app 自有目录，Claude/Codex 子进程与内置终端启动时注入 env，不再写 `~/.claude/settings.json` 与 `~/.codex/`——multi-agent open-q #9 按此解（codex provider 段由托管凭据生成，投影退役），打包版凭据来源随之与本机 CLI 配置解耦（与 D42 dev 态隔离互补）。具体设计进规格轮，本条只锁范围 |
| D48 | Codex CLI 选择功能立项（用户拍板 2026-08-16 当场两轮问答；multi-agent 阶段 3 立项） | ① **范围 = 全套 codeg 认可形态**：agent 选择入口（Composer 消费 `capabilities.agents`，首个 UI 消费者）+ 模型/思考档按 agent 适配（前置 = multi-agent open-q #5 模型目录半边，含「代理真实支持哪些模型」自建校验）+ 权限模式管理面（现仓仅只读展示，Claude permissionMode 与 Codex sandbox/approval 异构语义须统一表达）；② **绑定语义 = 零回合可选、物化后锁定**：会话 agent 在首条消息发出时物化进 wire 层与 session-index，物化后不可改（与 codeg `conversation.agent_type` 持久化同构，避免 per-runtime 历史/resume 链孤立），已物化会话 picker 转只读（具体形态进规格轮）；③ **三轴隔离纪律不变**：不改旧终端轴 `AgentPickerMenu`/`SessionBar`，聊天轴另立入口。具体设计进规格轮，本条只锁范围 |
| D49 | F2 超时看门狗体系重设计六项拍板（用户拍板 2026-08-18 当场问答，D48+T-10 点验分诊 F2 批；仲裁档 §15） | ① 确定死亡后草稿**自动放回**输入框（provenance marker 免误清）；② Host 崩溃**全量广播**判死所有 open 会话；③ **不付无据常量**（第二档阈值等一律不发明数字）；④ 接受 Codex 轴 TTFT 判死时远端回合可能续跑的残留；⑤ 渲染端半边**不加 flag**（接受规范 #6 偏离，回退性由 incident 夹具 `[I-1]` + 断言链承担）；⑥ 30min `SEND_WAIT_LOOP_BOUND_MS` 为**资源上界非判死权**。施工五片全落（S0 `06b4ac5`/S1 `2f1a7ea`/S5 `9e29e05`/S2 `7f6d9e2`/S4 `ad73ce8`/S3 `d614a83`），45s 误报体系退役 → 300s 活性可复位沉默天花板 + Host 双狗唯一 abort 权 |
| D50 | F5/F4/F6 可读性与 Composer 改造拍板（用户拍板 2026-08-18 对比稿问答：D1-b 1.625 行高 / D2-b 次要层提档 / **D3-c 与 D3-b 并用**（设计员明示反对后用户坚持，实现方否决权保留：§4.6 三条件，静态可判的「阅读宽度净损」施工时证否，余两条待 GUI）/ 不用背景图） | 施工四片全落（① `b1b8f6c` token 双档 7.20:1/6.70:1 + tool-arg 85% / ② `1c5a797` 密度换档+气泡非对称 `bg-accent` 85% 宽+助手中性容器 / ③ `1516623` Composer 两行布局+42px 契约退役 / ④ `19a65f7` 等待行俏皮动词+↑↓ 双计数+stalled 档+`(up to Ns)` 退役）。全量 vitest 239 文件 4724 例两轮全绿；GUI 点验清单 G-1~G-11+新增项随下轮点验批 |
| D51 | F11 重发后带图历史消息重复修复形态（编排链裁定：双盲评审对撞 → 仲裁三版台架+10 万行扫描；用户「四批全开工」授权内） | 修法 = 洞 A（游标回归 `anchorIndex+1` + `orderedIds` 定位空洞按 id 认领 + 对齐探针）合取洞 B（分档折叠身份 + 附件档三前置），失败方向恒「重复不丢失」；回退安全态仅 {两洞全退}/{洞 A 在位洞 B 退}。施工 `bc732f8`（48 判定 + 15 对变异零存活 + S-0'~S-2 发射证据链）。uuid 改道降 F11-b 另立票 |
| D52 | 2b 打包链立项两项拍板（用户拍板 2026-08-19 当场问答；multi-agent 阶段 4 立项调查收口） | ① **D36（Linux 捆随包 node）与 D41（打包前置 lint+typecheck+vitest 三门）都并入本批施工**——三者同在 build.yml/afterPack/verify 一条链，一次施工收口，Linux verify「Node 24 resolvable」随批转绿；② **macOS 不纳入本批**：范围 = win+linux 口径一致，mac 维持本地打包 + codex 走全局 npm fallback，mac CI 作业（含签名/公证）另立票。立项调查档 [2026-08-19-stage4-packaging-kickoff.md](2026-08-19-stage4-packaging-kickoff.md)（三路摸底 + REQ 清单 + 风险 R1~R6 + 切片草案 P1~P4） |
| D53 | 0820 使用反馈批三项拍板（用户拍板 2026-08-19 当场问答；分诊 FB1~FB11 全表见 [2026-08-19-usage-feedback-0820-triage.md](2026-08-19-usage-feedback-0820-triage.md)） | ① **FB2 copy 范围澄清**：加 copy 的是**正文 markdown 代码块**（模型输出给用户去终端执行的指令，如 git push）；工具调用 IN/OUT 块**维持防外泄安全边界不动**（`turnCopy.ts:6-10` 不改判）；② **FB7 Ran/Allowed 双行合并进工具行**（L 档，本批做，需打通 permissionId↔toolCallId 关联）；③ **FB9 LaTeX 本批做**（remark-math + rehype-katex，rehype 插件位解锁须过安全评审）。连同用户反馈原文即拍板的六项（FB1 渐进渲染【D26 改判】/ FB2 / FB3 气泡展开开关 / FB4 正文移出折叠组【T-31 §4.2/4.4 改判】/ FB6 状态行移底【T-31 §4.7 改判】/ FB8 时长格式化）构成本批施工范围；FB10 归用户机指引，FB5 答复数据无损，FB11 修后重评 |
| D54 | 打包链规格 §11 开放问题收口（用户拍板 2026-08-19 当场问答 + 编排者按规格建议采纳；规格 rev.2 [2026-08-19-stage4-packaging-spec.md](2026-08-19-stage4-packaging-spec.md)） | ① **Q6 PR 级门禁另立票后做**（本批按 D41 原文只做打包前置）；② **Q7 codex 0.145.0→0.147.0 升级票排 2b 收官后、flag 转 on 前**（blessing 重跑三件套）；③ 按规格建议采纳：Q3 CI 不跑 Codex 真回合（cch 事前报量铁律，S3 归手工/加密机点验）· Q9 refreshPath 本批不加闸 · Q10 GUI 侧 codex 路径逃生口另立 settings 票；④ Q1/Q2/Q8 归首跑观察项（continue-on-error → 回填 → 转硬门禁）。**阶段 4 施工开工（P1 先行）** |
| D55 | 0820 反馈批规格开放问题收口（用户拍板 2026-08-19 当场问答；规格 rev.1 [2026-08-19-fb-batch-spec.md](2026-08-19-fb-batch-spec.md)） | ① **底部状态行与 TurnFooter 合并成一行**（回合底部单条 meta 行）；② **Denied 警示色追认**（D28 编排者代拍项就此转正式拍板）；③ 编排者按规格建议采纳：FB1 段间距同档同值 · Codex itemId 本片不透传走安全回落 · copy 按钮常显低对比（F-B15 禁 hover-only）· PendingTurnHead 相邻观感转 GUI 点验项；④ **D53 ② 措辞勘误**：Claude 路径 permissionId 与 toolCallId 本就同串（`permissionBridge.ts:38-42` 有 toolUseId 即逐字返回，一次 P0 事故 + 回归钉为证），「需打通关联」实为「Claude 侧已同串 + Codex 侧另证」，FB7 的 L 档不确定性大半消除；⑤ 已吸收取证子档 fb4-fb6-structure-spec 删除（内容并入主规格）。规格 rev.1 转双轨双盲评审 |
| D56 | 0820 反馈批片③ GUI 点验后的形态拍板（用户拍板 2026-08-25 当场看图问答；规格 rev.2-b [2026-08-19-fb-batch-spec.md](2026-08-19-fb-batch-spec.md) §15 片③） | ① **Q14 收口 = C1「每个 answer 段各挂一个容器」可接受**，不降级 C3、不撤 D3-b —— **D50 保留给用户的实现方否决权本轮未行使、仍然开着**。⚠️ 用户当场订正判据：G-6 验的是「输出内容显示的**条理、逻辑和可读性** —— 该折叠的折叠，不该折叠的内容正常显示」，**不是数边框盒**；② **回合级折叠退役**：meta 行保留全部信息（`Worked for 24s · 2 tools · model · 相对时间 · copy`）但**去掉 chevron 触发器** —— 理由是每个 tool 行自己就能展开，而 FB4 把正文移出折叠组后，回合级折叠已无多少可藏。连带退役 `defaultTurnProcessOpen` / `TurnProcessOpenInput` / `collapsedLeavesNothing` / `hasUnresolvedPermission` / `turnHasFailure` 五个纯函数与 `[FB6-6]`/`[FB6-7]`/`[FB6-8]`/旧`[FB6-4]`/`[FB6-9]`/F5/F10/`[FB4-5]` 等断言（都只为那一个决策服务，留着即空壳）。**授权红线由「条件」升为「结构」**：过去靠首返回强制展开，现在 process 段无条件渲染、授权卡根本藏不了，新 `[FB6-4]` 以三条否定钉死；③ **工具行默认折叠**（恢复历史与日常对话一致）—— **推翻既有 sign-off ②「failures auto-expand」**，`ToolRows` 改 `defaultOpen={view.defaultOpen ?? false}`，T-34 的 LIVE subagent 面板成为唯一会默认展开者。落地 `4a1c8402`，全量 245 文件 / 4946 例全绿 |
| D57 | 0820 反馈批片④ G-10 拍板：**接受原生 MathML 排版，不转 KaTeX HTML 输出**（用户拍板 2026-08-25 看图后当场决定；规格 [2026-08-19-fb-batch-spec.md](2026-08-19-fb-batch-spec.md) §8.4 / §15 片④） | **背景**：§8.4 给了两条路 ——（a）`output: 'mathml'` 走 Chromium 原生渲染、**不需要 KaTeX 字体**；（b）KaTeX HTML 输出、排版更好但自带 60 个字体文件（实测 CSS 24.7KB → 1.46MB），**正面撞 `globals.css` 的 no-bundled-webfont 红线**，rev.2 的裁定 D-2 明确「（b）路不得自动转入，须另取立项/红线偏离授权」。**G-10 实测**：三条公式全部进 MathML（3 个 `<math>`、0 个 `.katex-html`），行内 `$` 零误伤；分数线初测坏，**排查后确认根因是 Chromium 的 `math` 泛型族在 Linux 上经 fontconfig 解析而 fontconfig 无 `math` 别名**，补 `--font-math` 系统回落链后完全正确（零 `@font-face`、零字体资产，不破红线）。**残留**：矩阵 `pmatrix` 的括号不随高度拉伸（Chromium MathML 的 stretchy operator / 字体 variants 限制），CSS 修不了。⇒ **用户拍板：接受。** 不启动红线偏离，(b) 路不走；矩阵括号形态记 [inbox 另立票](../plantree/ideas/inbox.md)。落地 `65ee39be` + `e0a3c136`，全量 245 文件 / 4962 例全绿 |
| D58 | **Codex 直接默认打开，退役 `AICLIENT_AGENT_CODEX` feature flag**（用户拍板 2026-08-26 当场问答，原话「直接打开就行了，要什么设置按钮变量的」） | **背景**：Codex 自 S3 slice 2 起就藏在 `AICLIENT_AGENT_CODEX=1` 后面（工程规范第 6 条「带 flag 上线、两个位置都跑」）。到今天它守的运行时已经完工 —— S3 六切片 + D48 四切片 + 2b 打包链全落，codex 二进制已随包。**真正决定退役的是另一半：这个开关用户够不到。** flag 唯一到达路径是 `AgentHostProcess.start()` 的 `{...process.env}` 继承 ⇒ 只有**从终端启动 Electron** 的人设得上；双击桌面图标 / Dock 点开的用户拿到的是桌面会话环境，**`~/.bashrc`、`~/.zshrc` 根本不被读**。而 `hostEnv.ts:26` 又刻意不注入这个 key（理由：不想有第二个地方决定 flag 含义），设置层也没有任何对应开关（全仓 settings 里的 `codex` 命中全是另外两根轴：终端 `BuiltinAgentId` 与一次性助手 `AIProvider`）。⇒ 在 Linux/macOS 上，装了安装包的用户**物理上无法打开 Codex**。一个产品用户够不到的开关不是灰度控制，是焊死的关闭键。**拍板**：删 `CODEX_FLAG_ENV` / `resolveCodexEnabled` / `flag_off` 三者，四道闸降为三道 —— 可用性此后**只由「关于这台机器为真为假的事实」决定**：凭据模式 × 入口可解析 × 隔离 home 可准备，每条都已有自己的 reason code。**连带行为变更（记档，非缺陷）**：`prepareHome()` 过去被 off 位短路（flag 臂刻意无副作用），现在**每次 Host 启动都会跑**，对从不使用 Codex 的用户也是 —— fallback 分支会建 `<userData>/codex-home/`、把用户 `~/.codex/config.toml` 投影进去、`auth.json` 更新则拷贝。**只读 `~/.codex`，从不写回**，且幂等，但不再免费。**实证**：零 env 起 Host（仅给 Main 本来就会注入的 `AICLIENT_CODEX_HOME` + 随包 `AICLIENT_CODEX_JS_PATH`），`host.ready` 广播 `capabilities.agents = ["claude-code","codex"]`，隔离 home 生成 `config.toml` + `auth.json`。四门全绿（typecheck 0 · biome 995 文件 0 · **vitest 247 文件 5003 例**）；变异 2/2 咬红（重新插入一道 env 闸 → 默认可用断言红；拿掉 `prepareHome()` 调用 → 探针调用次数断言红），md5 对账还原。**测试面连带**：`protocolErrors.test.ts` 的 harness 原本用 `AICLIENT_AGENT_CODEX: ''` 钉住 off 位，现改为四候选饿死（空 PATH + 不存在的 env override 与 node exec path），断言从 `flag_off` 改钉 `entry_missing` —— 否则本仓现在自带 codex，reason 会在不同机器间漂移（F13 当初抓到的正是这种漂移）。落地 `ea9fa962`。 |
| D59 | **统一凭据目录 + 托管凭据转默认，四项方向拍板**（用户拍板 2026-08-26 当场问答；立项见 [plantree/plans/unified-credentials](../plantree/plans/unified-credentials/README.md)） | **背景**：用户问「claude/codex/pi 的 env 怎么注入的」，并指出应有我们自己的目录、配置自持久化，而不是从 `~/.claude`/`~/.codex` 提取修改。**实测结论：想要的东西大部分已存在** —— `~/.aiclient/settings.json` 已在用；登录拿到的 url+key 已按 agent 分存在 `<userData>/credentials/vault.json`（`claude:{baseUrl,authToken}` / `codex:{baseUrl,apiKey}`，加 pi 即加第三个 arm）；`adoption.ts` 已能免重登收编存量。缺的是合并、改名、打开三件事。**四项拍板**：① **合并** —— 设置与凭据收进同一个我们自己的目录（今天分裂在 `~/.aiclient/` 与 `<userData>/credentials/`）；② **改名 `.aiclient` → `.pilab`**；③ **保留 safeStorage 加密**（用户问「有意义么」，据实答：防误泄不防被盗 —— 同用户下的恶意代码可直接调同一个 safeStorage，且 Linux 无 keyring 时 Chromium 退到固定密钥后端而 `isEncryptionAvailable()` 仍返 true，实际防护接近装饰；但文件已 0600/目录 0700，代码也已写好，删掉是净损失，故保留且**不作为安全底线依赖**）；④ **打开 `AICLIENT_MANAGED_CREDENTIALS`**，让 claude + codex + 未来 pi 统一用登录拿到的 key。**④ 与 D58 同型但更重**：D58 退役的 codex flag 只决定「广播不广播」，这个 flag 一开会把 Claude 的 `CLAUDE_CONFIG_DIR` 从用户真实 `~/.claude` 切到 `<userData>/claude-home`，且那份 `settings.json` 是 Main 从 vault **重写**而非合并。**故本次只拍方向、不开工**：改名的前置是先把 5 处重复的 `.aiclient` 字面量收敛成单一常量（`SharedSessionState`/`RemoteAuthBroker`/`RemoteConnectionManager`/`RemoteRuntimeAssets`/`RemoteHelperSource`）；两条核心未决项（dev/prod 是否共用凭据 · 存量用户 `~/.claude` 里的非凭据配置怎么办）见 [open-questions](../plantree/plans/unified-credentials/open-questions.md) #1/#2，未决前不进入施工 |
| D60 | **隔离 home 降级为「只隔离凭据」——取消 `CLAUDE_CONFIG_DIR` / `CODEX_HOME` 整体重定向**（用户拍板 2026-08-26 当场问答；触发原话「如果目前隔离目录的方案会导致 claude.md commands/ skills/ 等内容失效，那我认为我们可能需要重新思考方案」） | **背景**：用户追问隔离目录会不会让用户自己的 `CLAUDE.md` / `commands/` / `skills/` 失效。**取证结论：会，但源头是三条互相独立的机制，其中两条今天已经在失效**——① **CLAUDE.md 不进模型上下文**：源头是 `claudeRuntime.ts:985` 的 `settingSources: []`（SDK `sdk.d.ts:1889` 原文 `Must include 'project' to load CLAUDE.md files`），**与隔离目录无关**，是 D47 之前为避免 settings.json 的 `permissions.allow` 阴影 `canUseTool` 权限卡而刻意做的；② **用户级 `commands/`/`skills/`/`plugins/`/hooks 失效**：源头是 `CLAUDE_CONFIG_DIR` 整体重定向，`managedClaudeHomeStartup.ts:89-90` 只 mkdir 空目录不收养，plugins 连空目录都没有——今天被 flag 挡着，S3 转默认即全量暴露；③ **codex 全局 `AGENTS.md` 与 `agents/`/`hooks/`/`skills/` 整棵树静默消失**：`ensureCodexHome` 只搬 `config.toml`+`auth.json`，且**不受托管 flag 控制，今天已在影响用户**。另查出 GUI 内部口径分裂：`ClaudeCompletionsManager:29-43` 扫的是 `~/.claude` ∪ `CLAUDE_CONFIG_DIR` **并集**，而 `PromptsManager`/`PluginsManager` 只跟重定向后的单目录 ⇒ **补全框里看得见的 `/命令`、敲下去不生效**，比整体消失更难排查。**拍板（四选一中选 A）**：**只隔离凭据，不隔离人格**——取消整体重定向，凭据改由 Main 经 host env 直送 Host（照抄仓内已有的 codex 通路 `hostEnv.ts:161` `AICLIENT_CODEX_API_KEY`），终端 pty 同样直接注入；用户的 `~/.claude` / `~/.codex` 配置树原样生效。**论据**：重定向从来不是凭据注入的必要条件——Claude 侧今天绕了一趟磁盘（Main 把 vault 写成 `<claude-home>/settings.json` → Host `loadClaudeSettingsEnv()` 读回来合进 `options.env`），**正因为凭据经「文件」传递，才不得不控制「目录」，一旦控制目录就顺带劫持了整棵配置树**；而 D47 S0 的 [E3 spike](2026-08-15-d47-s0-spikes/e3-sdk-no-home-session.md) 已实证 SDK 侧凭据纯经 `options.env` 即可、假 HOME 事后为空树。落选：**B（隔离 home 做成软链投影）**——成本随 agent 家数线性增长，Windows symlink 权限、投影更新时机、两处编辑分叉三个长期坑，且与 D58「够不到的开关不是灰度」同构；**C（维持现状+UI 明示）**——把实现细节推给用户承担。**连带效果**：open-q #2（存量 `~/.claude/settings.json` 非凭据配置怎么办）**自动消失**（文件根本没被换掉）；#5 修法定为「取消隔离」而非「扩大投影」；`managedClaudeHomeStartup.ts:99-105` 的 CLAUDE.md 一次性收养逻辑可整条删除。**不在本决策内**：`settingSources: []` 要不要改成 `['project']` 让 CLAUDE.md 重新进上下文——那要另想办法压住 `permissions.allow`，**单独立项**，勿混入本批。**约束**：本次只拍方向不开工，S2/S3 按新形状重塑后再进 execute |
| D61 | **取消隔离后用户自有配置全部继承 —— 不保留 deny 清单**（用户拍板 2026-08-26 当场问答，原话「让它生效，然后开工 claude 那半」；由 [E1 取证](2026-08-26-s0-spikes/e1-codex-no-home.md) §6 带出，关闭 open-q #7） | **背景**：D60 拍板取消隔离 home 后，`ensureCodexHome` 今天 deny-by-default projection 刻意丢弃的用户配置（`developer_instructions` / `mcp_servers` / `notify` / `profiles` / `history`）**会流回来生效**。E1 R8 实测：用户 `[mcp_servers.usermcp]` 被真实拉起（sentinel 文件落地）。**能补的与不能补的**：安全档位可用 `-c` 强制盖住（R5 实测：用户 config 写死 `approval_policy="never"` + `sandbox_mode="danger-full-access"` 照样被覆盖成 `on-request`/`workspace-write`）；但 `-c` 是**合并进表不是替换表** —— `-c mcp_servers={}` 整表清空**无效**（R9），只有逐条 `-c mcp_servers.<name>.enabled=false` 有效（R10），而逐条压制**需要先读用户 config 枚举名字**，等于把刚拿掉的「读写用户配置」语义换个形式请回来。**拍板：选项① 全部继承**，不做逐条压制、不做 UI 明示以外的拦截。**理由**：deny 清单是隔离 home 时代的产物（那时分不清「用户的」与「我们的」，只能一刀切），D60 已判定隔离本身是错的定位 —— 继续保留 deny 清单等于保留半个隔离。风险面由**我们强制的 `approval_policy=on-request`** 兜底：用户接的工具真要动手仍需过审批弹窗。**已知代价（用户已知悉）**：本条**扩大了运行时实际会执行的东西** —— 用户自己接的 MCP 会真实启动，其 `developer_instructions` 会进入 agent 指令。**⚠️ 附带的诚实标注**（E1 已登记，非本决策内容）：`developer_instructions`/`notify`/`profiles` 与 `mcp_servers` 同文件同路径，按机制推断会一并生效，**但这是推断不是实测**；跨进程 resume 的 posture 是否仍由 `-c` 承载**未测**。两条都是 S0' 施工前置 |
| D62 | **S2 改名批：接受远端旧目录孤儿，不做远端搬运；改名写进发布说明**（用户拍板 2026-08-26 当场问答，原话「接受孤儿目录，写进发布说明」；关闭 open-q #3） | **背景**：S2 把 `~/.aiclient` 改名 `~/.pilab` 后，**已经连接过的远端机器上会留下旧目录孤儿** —— 远端设置回到默认、runtime 缓存重下一次。**不会坏**：远端 helper 是我们自己下发的，不是第三方契约；只是观感像「远端配置被重置了」。**拍板：选项①接受**，并在改名批的发布说明里写明这一项。落选②「远端也做一次搬运」：要在 helper 里加一份长期维护的迁移逻辑，而远端 runtime 本就是缓存、设置面也小，不值这个代价。**连带**：S2 的交付物多一条 —— **发布说明必须写明**，否则用户会把它当 bug 报上来。**至此 S2 开工前置全部出清**：#1 按 profile 分子目录 · #3 本条 · #4 目录名二次确认取 `.pilab`（用户当轮举例写了 `.aiclient`，反查确认后维持原拍板）。S2 目标形态（用户当场指定）：`~/.pilab/<profile>/{settings.json, session-state.json, credentials/vault.json, remote-auth/, remote-runtime/}`，把今天分裂在 `~/.aiclient/`（设置）与 `<userData>/credentials/`（凭据）两处的东西合成一处 —— 即 D59 ①「合并」的落地形态 |
| D63 | **「使用本机已有配置」探不到时放行，不置灰**（用户拍板 2026-08-27 当场问答，原话「探不到时放行，用户自己想办法去」；关闭 [entry-and-environment open-q #1](../plantree/plans/entry-and-environment/open-questions.md)） | **背景**：两按钮登录页的第二个按钮要判断「不注入时用户自己能不能拿到凭据」，而本机可能是三态（配好 / 什么都没有 / 只配了一半）。**拍板：放行**，探测结果不作为按钮可用性的闸。**落选「置灰 + 说明缺什么」**（本轮建议方向）。**已向用户明示并被接受的代价**：探不到而用户仍点进去时，症状是「界面一切正常，发第一条消息才失败」，且错误信息埋在 CLI 侧 —— 这是排查成本最高的一类。**连带要求**：既然不拦，**首条消息失败时的错误必须能落到 UI 上并说清缺什么**，否则本拍板就是把成本转嫁给用户；这条与 [E2 §对设计的含义 ③](2026-08-26-s0-spikes/e2-codex-resume-and-inherited-keys.md) 要求的「配置加载失败要带出文件路径+行号」是同一件事，合并处理。**连带简化**：探测本身可以从「能不能起一个空会话」的真探测降级为静态判断（不再承担闸的职责），成本大幅下降 |
| D64 | **凭据模式存进 `~/.pilab` 下的配置文件，成为运行期状态**（用户拍板 2026-08-27 当场问答，原话「存到我们 pilab 的配置文件」；关闭 [entry-and-environment open-q #2](../plantree/plans/entry-and-environment/open-questions.md)） | **背景**：今天「用哪套凭据」是构建期开关 `AICLIENT_MANAGED_CREDENTIALS`，两按钮登录页要把它变成用户的一次选择。**拍板：存进 `~/.pilab/<profile>/settings.json`**（S2 已把该文件搬到此处），不进 vault —— 「我选了什么」与「密钥是什么」是两件事，绑一起会让模式切换牵动凭据读写。**连带（重要）**：**这改写了 [unified-credentials S3](../plantree/plans/unified-credentials/roadmap.md) 的形状** —— S3 不再是「退役 flag」，而是「把 flag 换成一个用户可见的运行期状态」。两者必须同轮定，否则 S3 会先落一个马上要推翻的形态。**未定（留给 S3 同轮）**：能否中途切换、切换时在途会话怎么办 —— 最省事是「切换需重启」，热切要处理在途会话 |
| D65 | **启动探测只保留 git；agent 探测退出启动门禁；非 Windows 缺 git 只给提示**（用户拍板 2026-08-27 当场问答，原话「目前的探测感觉保留一个 git 就行了，非 windows 给个提示」；关闭 [entry-and-environment open-q #3](../plantree/plans/entry-and-environment/open-questions.md)） | **背景**：node / claude / codex 三样均随包（cometix 2.1.212 · codex 0.149.1 · `resources/node-runtime`），用户机器上有没有与对话能不能跑无关；`ClaudeRuntimeChecker` 已在 2026-08-26 按此改过一半，但 `resolveGateDecision` 里还留着一条来自系统 `claude --version` 的 `cliMissing` 分量。**拍板三条**：① 启动门禁的 agent 探测**全部退役**（连带 onboarding 的 `cli-check` / `cli-install` 两步）；② **git 检查保留**（本产品是 worktree 管理器，git 是真依赖）；③ 非 Windows 缺 git **只给提示**，不补自动安装（今天 `AgentInstaller` 的安装能力被 `ensureWindowsOnly` 限死在 Windows）。**⚠️ 落档时核出的范围修正（须写进施工票）**：退役的是**启动门禁那条链路**，**不是 `CliDetector` 本身** —— `AgentSettings.tsx` 仍在用 `cli.detectOne` 探测第三方与自定义 agent（droid / gemini / auggie / custom），那些**确实没随包**，探测是真需求；远端 helper 也暴露同名 RPC。一刀切删 `CliDetector` 会打断设置页 |
| D66 | **应用内终端的 codex 走用户自己的配置，不补公司网关**（用户拍板 2026-08-27 当场问答，原话「终端不走公司网关也没事」；关闭 [unified-credentials open-q #8](../plantree/plans/unified-credentials/open-questions.md)） | **背景**：S0' codex 侧落地后，应用内终端敲 `codex` 不再走公司网关。原因不是遗漏 —— 终端原先拿的是「`CODEX_HOME` 指向托管目录 + `AICLIENT_CODEX_API_KEY`」**两件套，而这两半拆不开**：key 单独给毫无意义（用户自己的 config 不会用我们这个 `env_key` 名字），而 codex **没有任何环境变量能改 `base_url`** —— provider 表只存在于配置文件里。托管目录一删（[D60](openchamber-chat-refactor-ledger.md)），这对整个塌掉。**与 Claude 侧的不对称不是疏漏**：`ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` 是 Claude CLI 直接读的名字，所以 Claude 的凭据不需要文件也不需要目录；codex 没有对应物。**拍板：选项①接受。** 落选②「给终端单独造一份配置」（又回到「我们写用户配置文件」的语义，正是 D60 拿掉的东西）· ③「提供一个 `codex` 包装命令」（等于在用户 PATH 前面插一个我们的同名命令，另有一套问题）。**连带**：这条与 D60「用户环境原样生效」同向 —— 终端本来就应该是用户自己的环境；agent 会话走网关、终端走用户自己的，这个分界是清楚的，不是妥协 |
| D67 | **`settingSources` 打开为 `['user','project']`，且完全照配置办 —— 不加任何覆盖**（用户拍板 2026-08-27 当场问答，两问两答：「只问有副作用的」+「两个都读，接受这个风险」，追问「配置文件里那份是什么就是什么样，我们为什么要去干涉它？」后定为**甲**；关闭 [unified-credentials open-q #6](../plantree/plans/unified-credentials/open-questions.md)） | **背景**：`claudeRuntime.ts` 传 `settingSources: []`，代价是**两份 CLAUDE.md 都不进模型上下文**，用户 `settings.json` 里的 env/hooks/model 也全不生效（[取证](2026-08-27-settingsources-spike/README.md) §A①）。当初设它的理由**经实测成立**：配置里一条 `permissions.allow` 确实会整个跳过 `canUseTool`，权限卡从不出现（§C3，且 C1/C2 证明那条规则是活的）。**拍板三步走**：① 权限卡「只问有副作用的」；② 两个来源都读、接受仓库风险 —— 而 §F 实测确认**仓库提交的 hooks 会真的执行**（对照：同一份文件想把 `defaultMode` 调宽会被 SDK 拒绝，hooks 没有这道防线）；③ 一度实现了 `managedSettings: { permissions: { ask: ['*'] } }` 把所有工具摁回权限卡（§D4 实测有效），**被用户当场推翻并去掉** —— 那版实现分不清「用户自己写的」与「仓库塞进来的」，防后者的代价是推翻前者，与 [D60](openchamber-chat-refactor-ledger.md)/[D61](openchamber-chat-refactor-ledger.md)「用户环境原样生效」直接冲突。**支撑甲的关键**：用户同一轮已接受**更大**的风险（hooks 能跑任意命令，连工具调用都不需要），为**更小**的那个去覆盖用户自己的文件站不住。**净效果**：权限卡行为与官方 Claude Code 命令行完全一致，我们不再比内嵌的工具更严。**过程中废掉两个中间方案并留痕**：工具名单方案（实测 29 个工具，随口举的三个名单会漏掉「访问网页」，且手工名单会随 CLI 升级腐烂、失效方向是放行，§G/§H）· 丙方案（用 provenance 只挡仓库那份，技术上可行但未采纳）。**连带（同轮追加）**：`'local'`（`<项目>/.claude/settings.local.json`）**也一起开**（用户原话「开吧，测完就改」）—— 该层被 CLI 自己的 `.gitignore` 挡在版本控制外，**只可能是用户自己写的**，归属与 `'user'` 相同、暴露面比 `'project'` 更小；§L 实测前后对照干净（规则只写在该层时，不开 1 张卡、开了 0 张卡），§M 用本仓真实文件复核（`outputStyle` 由读不到变为读到）。最终形态 `['user','project','local']`。**未决**：`managedSettings` 的 restrictive-only 过滤文档与实测不一致（本批未用它，不阻塞） |
...
| 2026-08-15 | **意外发现小批落地（含 #13 dark:）** — 四件全修：① 面包屑 button 嵌套改 render prop（全仓唯一违规）；② runtimeEvent 超订收敛（**取证改判：静态超订 11-13/10 非泄漏**——引用计数总线单上游扇出、8 订阅点换线，顺带修单 listener 抛错阻断全链）；③ 同步卸载告警六处禁区 unmount 延迟出 commit（react-dom 源码级机制取证；DiffViewer 取证 2 处现场实为 4 处，施工员行使否决权全修）；④ dark: `@custom-variant` 一行接回 `.dark`。新测试 18 例；**变异验证 7/7 咬合**（一轮 inert 换承重：disposed 守卫须「同 listener 重订阅后旧句柄二次调用」场景方咬合，补例后现红）；四门全绿 **vitest 166 文件 3357 例 0 红**；**CDP 复验四查全过**（面包屑菜单现场零嵌套 / dark: 机制翻转 / console 无三类告警 / 截图目检）。规格 + as-built：[smallbatch-spec](./2026-08-15-incident-smallbatch-spec.md) | ✅ | `5dab201` |
