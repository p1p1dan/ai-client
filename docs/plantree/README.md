...
|---|---|---|---|---|
| [openchamber-chat-refactor](./plans/openchamber-chat-refactor/README.md) | In Progress | **开发线全部收口 → 真机点验期**（2026-08-06 第十二轮点验后；观感对齐批次任务 T-29 / T-12~T-15 / T-23 / T-32 / T-16 / T-33 / T-35 / T-34 全部 Done）。Phase 0A 整体仍 🟡（A02/A03/A04 未立项） | **2026-08-08 测试机五问题修复批 `9a6cc01` + 版本抬 `0.4.0-test.2`（`ff63987`）**，四门全绿 2481 例 0 红。逐批明细见 [implementation-status](./plans/openchamber-chat-refactor/implementation-status.md) / [roadmap Done](./plans/openchamber-chat-refactor/roadmap.md) / [主线台账](../plans/ledger-claude-mainline.md)；2026-08-05 前活动状态原文见 [history 归档](./plans/openchamber-chat-refactor/history/) | ① **T-10/T-11 真机点验（当前主项）**：Windows 出包（[T-10 清单 M1~M4](../plans/t10-packaged-gui-checklist.md)）→ 测试机回归（[加密机测试方案](../plans/2026-08-06-encrypted-machine-test-plan.md)步骤 4b）→ 加密机现场 → CP2/CP5；② 开发线下一步待用户裁定（候选四路见 implementation-status Next Target）；并行残留：T-04 网关阻塞 · open-q #15/#22/#23 · T-21 截图 |
| [multi-agent](./plans/multi-agent/README.md) | In Progress | **S3 施工中——切片 0/1 已落地，下一件切片 2 Codex 客户端骨架**（2026-08-06 解冻，同日 S1 spike → S2 设计 → S3 开工四连） | **2026-08-06 S3 切片 0+1 `0314216`**（agent 绑定基座：`agentWire.ts` + 协议增量 #1–#19 + 绑定回流链；双轨对抗复核 1 blocker + 5 major + 2 minor 全闭环；新增第四道门 `typecheck:agent-host`）。S1 裁定不接 ACP 直连 `codex app-server`（[S1 报告](../plans/2026-08-06-s1-acp-codex-spike-report.md)）；S2 施工档 = [S2 设计档](../plans/2026-08-06-s2-codex-integration-design.md)。逐批明细见 [roadmap](./plans/multi-agent/roadmap.md)；长叙事原文见 [history 归档](./plans/multi-agent/history/) | **切片 2 Codex 客户端骨架**：JSON-RPC（服务端请求自有 id 空间从 0 起）+ 单一 `pendingServerRequests` 表（close 时遍历回掉）+ 单一 status mapper + 隔离 `CODEX_HOME`；后续 3 提问桥（4 条真实报文夹具回放 + `isSecret` 补掩码）→ 4 权限投影 → 5 历史 → 6 收口。flag `AICLIENT_AGENT_CODEX` 默认 off。细节见 [S2 设计档 §3](../plans/2026-08-06-s2-codex-integration-design.md) 与 [roadmap S3 表](./plans/multi-agent/roadmap.md)。未决：#4 git 参照点 · #5 模型目录半边 |
