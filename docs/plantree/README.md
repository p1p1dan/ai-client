...
| [pix/pi-app UI 对齐改造](./plans/pix-ui-alignment/README.md) | **In Progress** | 批次 5 已落地，下一步批次 5.5 | U08-2 思考档补齐 Pi 七档（`off`/`minimal`），并统一边界处三份五词拷贝——其中 `isWorkerEffort` 是发布级缺口（全仓 261 files / 4066 tests 全绿），[U08-2 evidence](./plans/pix-ui-alignment/evidence/2026-09-03-u08-2-thinking-levels.md) | U13 免绑定会话跨重启可见性（[D04](./plans/pix-ui-alignment/decisions/004-unbound-session-index-visibility.md) 已拍板边界）|
