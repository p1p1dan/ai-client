...
**runtime 侧新增 T39**：Windows 安装版加密环境回归，按 [D20](./plans/pi-backend-migration/decisions/020-windows-bundled-node-worker.md) 改用随包 Node + IPC；其他平台保持 utilityProcess。Pi-only 收敛的 T28–T38 已于 2026-09-05 全部关闭，
正式发布走 [rollout/rollback runbook](../pi-only-rollout-rollback.md)。
原有四条活动计划均在收尾（界面侧两条、现场反馈待办及 A 组），新增 T39 跟踪 runtime 修复与验收。
B 组资源与导入已完成并归档。A/B 两组最初立项
...
6. 2026-09-08 立项组（A 组活动，B 组已完成）：
   ...
   → [roadmap](./plans/resource-home-and-import/roadmap.md)。
   **两组的文件边界写在各自 README 的「与另一组的文件边界」小节，开工前必读。**
7. 需要架构边界时：[Pi-only plan](./plans/pi-backend-migration/README.md)
   ...
   / [D20](./plans/pi-backend-migration/decisions/020-windows-bundled-node-worker.md)。
8. 只读当前任务直接链接的 topic/evidence/history。
...
4. Decision 索引与 Active decisions：稳定方向和边界（架构侧以 Pi 计划的 D14/D15/D16 及 D20 平台修订为准）。
...
|---|---|---|---|
| [Pi-only application convergence](./plans/pi-backend-migration/README.md) | **In Progress** | T39：Windows GUI 随包 Node 修复、工具打包验证及加密机验收；历史 T28–T38 不重开 | [roadmap](./plans/pi-backend-migration/roadmap.md) · [问题报告](../../Windows加密环境GUI异常分析.md) |
...
| [A 组 · 用量可见性与目录韧性](./plans/usage-and-catalog/README.md) | **In Progress（收尾）** | A1/A2/A3 代码与自动化全部落地（55 files / 801 tests 绿）；只剩一次累计 GUI 点验与 A3 打包实测（首份真实快照已落地） | [TODO](./plans/usage-and-catalog/TODO.md) · [implementation-status](./plans/usage-and-catalog/implementation-status.md) · [roadmap](./plans/usage-and-catalog/roadmap.md) |

活动计划五条。A/B 两组最初按并行文件边界立项；B 组已完成，保留两份 README 中的边界登记供 A 组继续执行参考；
...
|---|---|---|
| [B 组 · 资源归位与会话导入](./plans/resource-home-and-import/README.md) | **Completed**（2026-09-08） | B1–B4 全部实现，303 files / 4532 tests 通过；真实 utilityProcess/TUI 与 Codex→Pi 联合取证通过。GUI 已交接累计轮次；源码提交 `40538e1e` 已合入 feat 开发分支 |
