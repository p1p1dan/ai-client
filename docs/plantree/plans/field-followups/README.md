...
> **状态**：In Progress —— 批次一（F07 / F11 / F06）、批次二（F08）与批次三（F09）已实现，
> 自动化门禁通过；GUI 点验、真实 HTTP 请求头与公告接口联调未验证。剩 F10。
...
## 已裁定的两个边界（用户 2026-09-07）

1. **公告铃铛统一放左侧栏底部**，不放标题栏。原文写「放在原『...』按钮的位置」，
   但 `WindowTitleBar` 在 macOS 上整体不渲染，照字面做会让一个已发布平台没有公告入口。
2. **公告端点不鉴权**（`GET {onboarding 服务}/api/v1/announcements`），
   好让登录已过期的人也能收到解释原因的那条公告。代价：该端点永不得承载用户相关或机密内容。
