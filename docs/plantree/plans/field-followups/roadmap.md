...
- **已裁定（用户 2026-09-07）**：铃铛**统一放左侧栏底部**（`UserFooterPill`），
  不放标题栏——`WindowTitleBar` 在 macOS 上整体不渲染，只放那里会让一个已发布平台没有入口。
  接口为**不鉴权的公开端点** `GET {onboarding 服务}/api/v1/announcements`，
  好让登录已过期的人也能收到解释原因的那条公告；代价是此端点永不得承载用户相关或机密内容。
- **落地**：`shared/announcements.ts`（四个纯函数）+ Main 服务（拉取/缓存/已读/单飞）
