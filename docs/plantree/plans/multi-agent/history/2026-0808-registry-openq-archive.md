# 归档 · multi-agent 注册表行 + open-questions 快照（2026-08-08 瘦身前）

> 2026-08-08 规划树整理归档时快照移入（archive-only，不再更新）。含两部分：
> ① [plantree 根 README](../../../README.md) 注册表行原文（S1/S2/S3 切片 0/1 长叙事——
> 同等信息在 [roadmap](../roadmap.md) 与 [S2 设计档](../../../../plans/2026-08-06-s2-codex-integration-design.md) 均有权威版）；
> ② [open-questions.md](../open-questions.md) 全文（含已关闭 #1/#2/#3 与 #5 权限半边的结项推导原文）。
> 现行状态一律以活动文件为准。
> **链接注**：快照内相对链接已按归档位置整体调深一级（仅路径前缀，正文逐字未改）。

---

## ① plantree 根 README 注册表行原文（multi-agent 行）

| [multi-agent](../README.md) | **In Progress** | **S3 施工中——切片 0/1 已落地，下一件是切片 2 Codex 客户端骨架**（2026-08-06 同日四连：解冻 → S1 spike → S2 设计 → S3 开工；用户裁定「自行规划，继续执行 S2，可以并行」「开工吧」）——用户 2026-08-06 裁定「multi-agent 支线解冻，开干」，2026-08-05 的「后置」裁定至此解除；解除前提已满足：主线开发线五任务（T-32/T-16/T-33/T-35/T-34）已于 2026-08-06 第十二轮点验全部转 Done | **2026-08-06（最新）S3 切片 0+1 落地 `0314216`——agent 绑定基座**——切片 0（零逻辑）新建 `agentWire.ts` 叶子模块（`AGENT_WIRE_NAMES=['claude-code','codex']` 为持久化 ABI，三轴分离写进文件头）+ 协议增量 #1–#19 一次性落完（**协议版本仍为 1**）+ 19 例 AST 静态扫描；切片 1（全局串行前置）正向链五跳 + Host 运行时自报身份 + **早退守卫放宽**（不放宽则 agent 归属永远进不了索引且不报错）+ 物化唯一点在 renderer `mergeSessionIndex`（磁盘老行保持无该字段）+ 侧栏平行加 agent chip。**双轨对抗复核 1 blocker + 5 major + 2 minor 全闭环，两轨互补显著**：blocker（Host 不读 `payload.agent`，codex 请求静默跑成 Claude，**resume 会把 Codex threadId 喂给 Claude SDK**）与 `registerSession` 缺口**仅 Codex 见**；typecheck 盲区（`exclude` 含 `src/agent-host/**`，实测根门编译 **0** 个文件、单独跑 **8 个真错**）与自报身份零覆盖**仅 Opus 见**；静态扫描形同虚设**双轨同判**（整文件豁免/文本共现/正则 import/**负控跑规则副本**，全量重写为 AST + 8 组变异实验）。**新增第四道门 `pnpm typecheck:agent-host`**（266 文件），理由：切片 2 要在 agent-host 写全新 `codexRuntime`。编排者独立变异复验：拆守卫恰好 2 例红、对照例绿、还原后 diff 无差异。四门 lint ✅ / typecheck ✅ / typecheck:agent-host ✅ / **2471 例 3 红**（Windows 既有失败，零回归，2426→2471）。 **同日更早 S2 直连接入设计收口 `7fa1270`**——四项并发（a 提问实测 / **b 绑定口径 Opus+Codex 双轨双盲** / c 权限投影 / d 历史降级）→ Reconcile 合流出单一施工档 [S2 设计档](../../../../plans/2026-08-06-s2-codex-integration-design.md)（389 行）。**关闭 #2 / #3，#5 关权限半**。**a 结 #2**：抓到 **4 条真实 `item/tool/requestUserInput` 报文 / 10 颗问题**，答复走通回合续跑（4/6 回合）；提问是**自包含报文不需 itemId 关联**（与 fileChange 审批正相反）；**`{answers:{}}` 是干净取消**，与 Claude 侧行为**相反**，照搬防呆会把 cancel 做死；「薄适配」由 [推测] 40–80 行校正为 **Host 100–150 + 类型 12 + 渲染端 0**（偏乐观约 2 倍，方向不变）。**证伪编排者线索**：三个 feature 开关任意组合外发请求体**逐字节相同**，真因是模型走 Codex 出厂提示词的散文后门。副产物**零额度看工具表法**，已取得 U4 负结果。**b 结 #3**：双轨独立收敛 6 处（**两轨各自命中同一个早退守卫** `SessionIndexService.ts:139-142`——遇无 runtimeIdentity 的 `session.created` 即早退会静默吞掉新字段）；分歧 3 处——**wire 名经用户裁定 `'claude-code'/'codex'`（不可逆）**、字段名取 `agent`、**回落点取 renderer `mergeSessionIndex`**（编排者复核发现 Codex 主张的 Main 侧 `ensureLoaded` 与它自己的否决项打架：`flush()` 写整张表 → 读侧规范化 = 不可逆写迁移）。**仲裁员挖出三份设计共同错误前提**：仓内已有 `BuiltinAgentId`/`AIProvider` 两套 agent 词表 + `agentId?` 已占用 + `'claude-code'` 已有 24 处 → b 原定静态扫描**落地即红**，已改写为三条可执行规则。**编排者复核新增**：`clientInfo.name` 会被 codex 揉进 User-Agent **发给 OpenAI**（对外身份 ≠ wire 名）；`isSecret` 推翻原案按 codeg 先例 + T-35 立场**补掩码**。协议增量 28 项验算后**协议版本仍为 1**。三门：lint ✅ / typecheck ✅ / test 2426 例 3 红（与 HEAD 逐例相同，零回归）。 **同日更早 S1 spike 四路探针实测**——产出 [S1 报告](../../../../plans/2026-08-06-s1-acp-codex-spike-report.md)（474 行）+ 三个探针脚本 `src/agent-host/spikes/s1-*.ts`（`bc531c7`）；**结 open-q #1 → 裁定「不接 ACP，直连 `codex app-server`」**。三条支撑：① 用户答「不加第 3 个 agent」→ ACP 价值不成立；② 实测**直连反而更便宜**（540–740 行 / 2.5–5.0 人日 vs ACP 670–1090 行 / 3.0–7.0 人日），原「≈相等」假设的复核条款未触发；③ **推翻 #1 隐含前提**——`codex app-server` 命令审批与补丁审批**已在真实回合捕获原始报文**，「直连不可行时 ACP 是唯一退路」不成立。头号实证：**ACP 只是把直连 payload 原样塞进 `_meta.codex` 转发一次**（两路独立跑、逐字段并列坐实），并丢掉 `applyNetworkPolicyAmendment`/`granular`/`approvalsReviewer`；代价另有 **362M node_modules（341M 是与 PATH 同版 codex 0.145.0 的纯副本）+ 3 级进程链 + 审批数据无 schema**。「接 ACP 就不用写解析器」被 codeg 证伪（它接了 ACP 仍用 **508 行** `emit_conversation_update` 装 13 分支）。同轮**校正 reuse-boundary 六行初判表**（3 确认 / 2 校正 / 1 部分推翻 / 1 未覆盖）+ 补两行新层，并推进 #2/#3/#5。**编排者逐条回验**四路探针结论，更正 **5 处引用错误**（含 `toolCard.ts` 整段路径错）+ 新发现 `DELEGATION_TOOL_NAMES` 有两份副本。三门：lint ✅ / typecheck ✅ / **test 3 例失败经 `git stash -u` 退干净 HEAD 复验为既有失败**（与主线台账「三绿」记录对不上，已另行提出）。**未闭合：#2 真实提问报文两条路都没诱发出（9 个真实回合零命中）**。 **2026-08-05 立项并补落** 2026-08-04 会话的调研结论三篇 topic：[acp-decision](../topics/acp-decision.md)（ACP 成本曲线：省的是解析器、从第 3 个 agent 起摊平；**Claude 线不走 ACP** 的证据链——`@agentclientprotocol/claude-agent-acp` 本质是我们 `src/agent-host` 的第三方版本，同一个 Claude Agent SDK）· [reuse-boundary](../topics/reuse-boundary.md)（问答卡上层 agent 无关，**仅 `questionBridge.ts` 303 行 Claude 专属** = D2 红利）· [codeg-reference](../topics/codeg-reference.md)（参照物在本机 `/home/dan/projects/codeg`；12 内置 agent 全经 ACP 适配器，pin `claude-agent-acp@0.64.1` / `codex-acp@1.1.9`） | **S1 spike**：`codex-acp` 能否拉起最小回合 · ACP 映射 vs 直连解析**两条路都要估** · Codex 提问形状（校正 reuse-boundary 初判表）· 模型目录 · 权限语义。出口 = 结 open-q **#1**（判据：三个月内加不加第 3 个 agent）→ 才谈并入主线。**S3 切片 2 — Codex 客户端骨架**（下一件）：JSON-RPC 客户端（**服务端请求走自有 id 空间且从 0 起**，与自发 id 混用会串号）+ **单一 `pendingServerRequests` 表**（提问与三类审批共用，`session.stop/close/shutdown` 必须遍历回掉全部挂起请求否则回合永久卡在 `waitingOn*`）+ **单一 status mapper**（`waitingOnUserInput` 与 `waitingOnApproval` 是同一条 `thread/status/changed` 的两个 flag，两个桥都不得自行发 `session.status`）+ `codexRuntime.ts` 与 `CODEX_PERMISSION_DEFAULT` 显式下发 + **隔离 `CODEX_HOME`**（不隔离必踩 `developer_instructions` 坑：模型口头答应却零 tool_call）。**验收**：`thread/start` 参数 == `session.created` 回显（仿 `claudeRuntimeOptions.test.ts` 的单一真相测试）· `session.close` 后 pending 表 size===0 且每条挂起请求都收到过 settle · status mapper 覆盖 `activeFlags` 四种组合。后续 **3** 提问桥（4 条真实报文夹具回放 + **`isSecret` 补掩码**）· **4** 权限投影 · **5** 历史先档 A 后档 C · **6** 收口（flag on/off 双跑 + 侧栏窄宽截图 U8 + 台账）。~~**0** 类型与断言骨架 / **1** 绑定回流链~~（**已落地 `0314216`**）· **2** Codex 客户端骨架（JSON-RPC + 单一 pending 表 + **单一 status mapper** + 隔离 CODEX_HOME）· **3** 提问桥（用 a 抓到的 4 条真实报文做夹具回放）· **4** 权限投影（decisions 表 + approvalCorrelator + 卡层，与 3 同批卡文件不并行）· **5** 历史先档 A 后档 C · **6** 收口（flag on/off 双跑 + 侧栏窄宽截图 + 台账）。flag = `AICLIENT_AGENT_CODEX`（默认 **off**，只控 `capabilities.agents` 与运行时注册，**不控协议字段/store 形状/渲染分支**）。细节见 [S2 设计档 §3](../../../../plans/2026-08-06-s2-codex-integration-design.md)。**剩余未决**：#4（git 对齐参照点缺失）· #5 模型目录半边 · U8 侧栏窄宽需截图 · 1 个真实回合留存不预支。**并行第二批（与 ACP 无关，可平移主线）**：重试横幅（数据已在 `chatSessions.ts:85`）→ 子 agent 实况（开 `forwardSubagentText`，`sdk.d.ts:1619`）→ stderr 进 UI。**已出本 plan**：三条能力缺失（重试横幅 / 子 agent 实况 / stderr 进 UI）2026-08-05 正式平移主线为 **T-33 / T-34 / T-35**——只用 Claude 直连链已有数据、与 ACP 判断互不依赖，压在后置支线下会被一并冻住。**Deferred**：多 agent 协同（用户明示先放一放） |
