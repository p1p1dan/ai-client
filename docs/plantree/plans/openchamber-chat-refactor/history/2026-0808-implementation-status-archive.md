# 归档 · implementation-status.md 全文快照（2026-08-08 瘦身前）

> 2026-08-08 规划树整理归档时自 [implementation-status.md](../implementation-status.md) 整文件快照移入（archive-only，不再更新）。
> 快照动机：开发线七任务已全部 Done（2026-08-06 第十二轮点验收口），活动文件里的逐批 Landed/Verified
> 记录与六份已结项点验清单（0-duodecies / 0-quindecies【合并版】/ 0-sexdecies / 0-septendecies /
> 0-octodecies / 0-novodecies）转为历史证据。权威明细始终在[主线台账](../../../../plans/ledger-claude-mainline.md)各行。
> **注意：本快照含少量当时仍活动的条目（用户线残项 / Handoff Notes），其现行版以活动文件为准。**
...
> 短操作交接。历史证据勿堆此处，进台账档案。2026-08-03 前的逐批 Landed/Verified 摘要与已结项点验清单原文已归档 [history/2026-0728-0803-archive.md](./2026-0728-0803-archive.md)。
...
- **Last Landed**:
  - 代码（2026-08-07 施工 / 2026-08-08 提交，**最新**）：**测试机用户反馈五问题诊断修复 `9a6cc01`**——M1 附件消息历史重放重复（`historyReplayMerge.ts` 新增 `isReplacementFoldable` + replacement fold：带附件的 runtime 副本替换 history 行位置而非两者都留）· M2 超时太短（`SEND_TIMEOUT_CEILING_MS` 115→180s，`HOST_STALL_TIMEOUT_MS` 120→195s，三层不变量 TTFT<renderer base<stall 维持）· M3 OpenChamber 壳缺仓库移除入口（App→WorkspaceShell→LeftNav plumbing + FolderMinus 按钮 + AlertDialog 确认弹窗；dead button 测试曾红→改 onClick-based Close）· M4 侧栏不显示 git 分支（`useWorktreeListMultiple` query `retry:false`→`retry:1, retryDelay:1000`）· M5 对话显示混乱（判定为 M1 重复消息的视觉后果）。九文件改动，测试 replayMerge 26/26 · attachmentLimits 28/28 · workspace-shell 302/302 · typecheck 干净。**提交前复核修正 M3 一处自引入缺陷**：folder→repository 原按显示名 / 路径后缀猜键（`my-app` 会命中目录名 `app`，删除动作上不可接受），改用建树同款 `projectIdForRepo()` 精确取键。四门 lint ✅ / typecheck ✅ / typecheck:agent-host ✅ / vitest **2481 例 0 红**。**下一步：出 Windows 包（T-10 出发前必做 M1~M4 仍全部未做）→ 测试机实测验证，重点 M4 分支显示**
  ...
  - 验收（2026-08-06，最新）：**第十二轮 GUI 点验用户验收通过（「点验完毕，归档当前进度」）——T-16 / T-33 / T-35 / T-34 四任务转 Done，开发线五任务全部收口**。布局模型三轮改判随验收定稿：T-32 降级梯 → round-11 右缘裁切（观感被推翻）→ **round-12 rail 常驻切换器 + 面板宽度压缩（现行，`cfc3bc1`）**；性能裁定成立（卡顿主因 VM 无 GPU 软渲染，实现侧拖拽 per-frame React 渲染与 width 过渡 reflow 已消除，真机指标归 T-10 清单）。0-sexdecies / 0-septendecies / 0-octodecies / 0-novodecies 四清单收口。明细见[主线台账](../../../../plans/ledger-claude-mainline.md) 2026-08-06 验收行
  - 验收（2026-08-05）：**第九轮 GUI 点验用户验收通过**（原话「点验通过，进入下一任务环节」）——点验清单 **0-quindecies【合并版】**（7 组 24 项）收口，**T-32 / T-12~T-15 / T-23 三任务一并转 Done**。随验收生效的追认四项：`Ctrl/Cmd+1..4` 改绑 1=git 2=files 3=context 4=terminal · `Ctrl/Cmd+B` 侧栏收展 · 四死按钮走删除支 · 72% 假 usage 环撤除。**遗留两项（不阻塞，已另有归口）**：① A2「Win10 字重真机核验」本机（Linux）不可采集 → 并入 [T-10 打包版清单](../../../../plans/t10-packaged-gui-checklist.md)（与 T-24 真机项同处）；② macOS 无标题栏行 → 全 app 无 usage 展示，既有缺口维持 backlog。验收态 HEAD `0b90baa`，工作树干净。明细见[主线台账](../../../../plans/ledger-claude-mainline.md) 2026-08-05 验收行
  ...
  - 更早批次（2026-07-28 ~ 2026-08-03 三批：五连修/T-21/T-22/T-26/T-27/T-28/T-05/T-19/T-30 批1+批2/第二~六轮点验修复等）：摘要原文见 [history 归档](./2026-0728-0803-archive.md)，明细见[主线台账](../../../../plans/ledger-claude-mainline.md)各行
- **Last Verified**:
  ...
  - 更早复核记录（2026-07-28 基线 51 文件 590 例起，全程只增）与 T-21 复核口径注记：见 [history 归档](./2026-0728-0803-archive.md)
- **Next Target**（2026-08-06 廿三次修订，第十二轮验收收口后）：**开发线无在建施工项——七任务（T-32 / T-12~T-15 / T-23 / T-16 / T-33 / T-35 / T-34）全部 Done，下一步走向待用户裁定**。候选四路（不互斥）：① **T-21 收尾**——唯一残留 In Progress：默认主题下中英混排三场景 + 6 处 `normal-case` 豁免的截图验收（open-q #10，用户线，出截图即转 Done）；② **multi-agent 支线解冻**——2026-08-05 后置裁定的前提「先把现有 Claude 客户端任务大致完成」已成立；③ **Deferred 复活**：C-17 问答/子 agent 进历史协议（T-34 已铺好修法抓手 `toolUseResult.agentId`+`outputFile`，同时解 D20 问答卡与子 agent 实况两处 resume 缺口）· T-25 旧模块原色清理（依赖 T-21 验收）· 后置 6 surface（pr/diff/plan/notes/browser/preview）· C-12 千 block 压测；④ **backlog 批量清票**（CI 出包缺口 ≈6 行补法已备 / T-29 转入四项 / 其余小票见下）。**已结清**：第十二轮点验验收通过 → 0-sexdecies / 0-septendecies / 0-octodecies / 0-novodecies 四清单收口，**T-16 / T-33 / T-35 / T-34 四任务转 Done**（布局模型三轮改判定稿「rail 常驻 + 宽度压缩」`cfc3bc1`；性能裁定成立：VM 软渲染主因 + 实现侧拖拽 rAF 直绘已治理）；第九轮点验验收通过 → 0-quindecies【合并版】收口，**T-32 / T-12~T-15 / T-23 三任务转 Done**；resume 两项优先项亦已结清。**残留（用户线，与开发线并行）**：T-24 真机 Windows 打包版 + **A2 Win10 字重真机核**（同归 [T-10 清单](../../../../plans/t10-packaged-gui-checklist.md) 第 8/9 项）；0-nonies ⑪ 真机指标；网络环境复测（open-q **#22**）；授权 FIX 3（open-q **#23**）；**T-04 网关阻塞**与 **#15 缓存复测裁定**并行。**backlog**：CI 出包缺口（build.yml `workflow_dispatch` 无 installer artifact 上传，≈6 行补法已备）；`agentWireStatic.test.ts` AST 扫描超时余量不足（单跑 3057ms / 限 5000ms，满载并发下会假红，2026-08-08 实测二跑即绿）；历史侧回合时长源；`ran N command(s)` 聚合复议（需 A07 基线修订）；T-29 转入四项（全局 `color-scheme` / 三 HighlighterCore 单例 / monacoSetup 懒化 / `isTurnActive` 死导出）；useGitChangeCount 的 useRepositoryStore 全局单槽副作用小票；sessionRuntimeFacts 重启会话权限行永「未上报」（已知限制）；gitQueryKeys fileDiff path 参数未归一化（注释登记）；隐藏终端随他人 surface 宽度重排 / compact 跨组拖放静默 no-op（均登记不修）；macOS 无标题栏行 → 全 app 无 usage 展示（既有缺口显性化）；**T-16 连带**：旧壳（legacy 标签页壳）本身的功能完整性从未在 D18~D27 各批中回归过，本轮点验若发现旧壳残缺，按「旧壳是回退不是产品」口径记 backlog、不即时修。
...
> 2026-07-28 重排版开发线原文（T-24~T-23 六项，均已落库或后置）见 [history 归档](./2026-0728-0803-archive.md)。
...
6. ~~**T-16 新旧壳开关成熟化**~~ —— **✅ 2026-08-05 代码落地 `fd57ebf`；2026-08-06 第十二轮统一点验验收通过转 Done（0-sexdecies 收口）**。原文保留供参照：拆 `App.tsx` / `Root.tsx` **两处强制覆盖**（**as-built 更正：实为四处**），`devFlags.ts:10` 的 `SKIP_ONBOARDING_GATE` 维持 `true`、**不作为达成手段**；验收三条（关开关重启仍旧壳 / 新旧壳可来回切且各自核心流程可用 / 开关状态持久化经重启）见执行计划 §3 T-16 行。

7. ~~**T-33 网络重试横幅**（0.5d）· **T-35 Host stderr 进 UI**（0.5d）~~ —— **✅ 2026-08-05 代码落地 `ba6b371`（T-33）+ `5a99ee1`（T-35）+ Codex 复核修复批 `e602096`+`d3635d3`+`8bf4829`（首轮 4 major+2 minor、终验第二轮 4 项，全闭环；T-33 门语义两度收紧）**；**2026-08-06 第十二轮统一点验验收通过转 Done（0-septendecies 收口）**。三条均 2026-08-05 由 multi-agent 支线正式平移主线（只用 Claude 直连链已有数据，与 ACP 判断互不依赖）。

8. ~~**T-34 子 agent 实况**（1.5d）~~ —— **✅ 2026-08-05 代码落地 `5c02730` + 首轮复核修复批 `a81cae7`（Codex 3 major + 4 minor 全闭环）**；**2026-08-06 第十二轮统一点验验收通过转 Done（0-octodecies 收口）**。验收①按令先行：probe 五场景实测定形（委派工具实名 `Agent`、默认态已下发子 agent tool 流、text/thinking 需 `forwardSubagentText`、四个 `system/task_*` 控制事件、写型权限主/子同桥且 options 带 `agentID`）→ Opus+Codex 双轨设计独立并行 → 仲裁合并（DR 骨架 + Codex 护栏）→ 半场分工施工。验收②在 Host 数据源修复（分流负向断言钉死）；验收④以 fixtures 事件预算断言可回归表达；**C-17 依实测改写（重开全丢非平铺，sidecar 亲验）**。→ 执行计划 §3 T-34 行 as-built

> **开发线现行顺序（用户 2026-08-05 裁定）= ~~T-32（已验收 Done）~~ → ~~T-16~~ → ~~T-33 / T-35~~ → ~~T-34~~（后四者 2026-08-06 第十二轮统一点验验收 Done）——**开发线五任务全部收口**；T-25（旧模块原色清理，依赖 T-21）仍后置，归 [roadmap Deferred](../roadmap.md)。**multi-agent 支线后置**（原定并行，同日改判为「先把现有 Claude 客户端任务大致完成」）。

### 用户线（点测 / 待拍板，与开发线并行）

0. **✅ 点验清单 0 ~ 0-undecies 全部结项**（第二~八轮，2026-07-30 ~ 2026-08-03；唯 **0-nonies ⑪ 真机指标未采集**仍待用户），清单原文见 [history 归档](./2026-0728-0803-archive.md)。
...
0-sexdecies. **✅ 点验清单结项（2026-08-06 第十二轮验收通过）——T-16 新旧壳开关（2026-08-05 落地 `fd57ebf`）**
...
0-septendecies. **✅ 点验清单结项（2026-08-06 第十二轮验收通过）——T-33 网络重试横幅 + T-35 Host stderr 进 UI（2026-08-05 落地 `ba6b371`+`5a99ee1`+复核修复批 `e602096`+`d3635d3`+`8bf4829`）**
...
0-octodecies. **✅ 点验清单结项（2026-08-06 第十二轮验收通过）——T-34 子 agent 实况（2026-08-05 落地 `5c02730`+复核修复批 `a81cae7`）**
...
0-novodecies. **✅ 复验清单结项（2026-08-06 第十二轮验收通过）——第十轮六项修复 + round-11/12 布局改判（`e856622` → `a005762` → `cfc3bc1`）**
