...
- **Last Landed**:
  ...
  - 代码（2026-07-31 二批）：**T-30 批2 施工落地（D25 分域字体 + Composer 形态对齐 Cursor，S0~S7 全序）`4735a45`（55 文件 +2072/−468）** —— 断言先行 18+ 条（fontDomainGuards A1~A6 / composerFormClasses F-A1~A22）→ `globals.css` 分域两栈 + 五新 token + A/B flag → 形态主体（42px pill 卡 + 中性聚焦梯度 + 24px 单一控件档 + **合并 `Sonnet High ⌄` 控件（删 ModelSelect/EffortSelect）** + ⊕ 接 @ 文件搜索（T-28 偏离①结清）+ 8px 净空归一 + empty 状态行按需显隐）→ `argKind` 双域工具行参数 + `<Ident>` 原语 → chat/workspace-shell 域映射全量清零（text-xs 等四类禁用字号）→ A07 v4 追记一~十一节一次成文 + phase0a/design-system 同批改写 → a09 凭证页骨架。三绿：typecheck 干净 / biome src 0 诊断（a08 外来诊断豁免，本轮实测含 5 errors 亦在其内）/ vitest 76 文件 **1403** 例（+58，3 例 Windows-only 豁免不变）。红线 `chatSessions.ts`/`runtimeEvents.ts` 零改动。**残留：a09 三测点截图 + 量化八项 GUI 实测随第五轮点验（清单见用户线 0-novies）**。明细见主线台账 2026-07-31「T-30 批2 施工落地」行
  - 文档（2026-07-30 二批）：**T-30 批2 施工依据——D25 分域字体规格交付 + 编排者三条临时裁定（已生效）**：deep-reasoner 产出 637 行规格（scratchpad `d25-font-design.md`）——系统字体栈零随包/Latin 先 CJK 后（禁 `system-ui`）；「内容 vs 控件」判据；`ToolRowView` 需加 `argKind` 字段区分参数渲染；`ToolRows.tsx` 两处 `<pre>` 缺 `font-mono` 属回归必补；字重仅 400/600 全平台真档（Win10 Segoe UI 无 500）；字距四档 + CJK 禁负值；~14 处补 `tabular-nums`；阅读栏 48rem→45rem（DPR 标定实测，对齐 Cursor 参照 45rem=48 CJK 字/行）；A07/phase0a 渲染层冻结段 + v4 追记；与 open-q #12 正交。量级估 ≈3.25~3.75d。**编排者三条临时裁定（①§2.3 三处细化点采方案 A；②阅读栏 45rem container token；③新建 `a09-font-domain-baseline.html`）已生效并随 2026-07-31 施工批全部落地**。明细见主线台账 2026-07-30 D25 规格交付行。
...
- **Last Verified**: 2026-07-28 Linux 三绿——typecheck 干净 / lint 609 文件 0 诊断 / vitest **51 文件 590 例**（3 失败=Windows-only 基线）
  ...
  - **2026-07-31 二批复核（T-30 批2 施工落地合并态）**：typecheck 干净 / biome **src 0 诊断**（`docs/design/a08…html` 外来诊断为 `beb8ccc` 引入、本轮未触碰——本轮实测其含 **5 errors** + 29 warnings + 3 infos，较上轮「0 errors」登记口径的差异如实记录，仍按既往豁免）/ vitest **76 文件 1403 例**（较 `cb2d8d7` 基线 75 文件 1345 例 +1 文件 +58 例；同 3 例 Windows-only 失败）。
...
- **Next Target**（2026-07-30 四次修订，第二轮 GUI 点验反馈闭环 `b159e4a` 已落地）：**T-27/T-28/T-19/T-30批1 第二轮 GUI 点验暴露的九条反馈已修复落库 `b159e4a`**（授权卡不渲染/排队消息丢失/模型直通/附件回显四项功能根治 + 网络重试可见性 + 视觉六项），**待用户第三轮 GUI 真机复验**（回归点验清单见下方用户线 0-octies）；**T-30 批2 解禁**——编排者三条临时裁定（D25 §2.3 细化点/45rem 阅读栏/a09 基线）用户本轮点验未异议，**裁定 A 生效，可开工**；开发线其余顺序不变——**T-24 收尾**（S0：全新机器 GUI 实测 + 台账补登，代码已随 `b38017b` 落库）→ **T-12/T-13/T-14/T-15**（四 surface，均依赖 T-22 已清）→ **T-23**。**残留待办**：网络环境复测需用户配合（open-q **#22**）；授权 FIX 3（`PendingPermissionDock` UX）留档（open-q **#23**）。用户侧 GUI 点测（含**多轮上下文**必测项）与 **T-04 网关阻塞**、**#15 缓存复测裁定**并行。
...
0-octies. **第二轮点验修复回归点验（2026-07-30 新增，`b159e4a`）**：① 授权卡正常渲染不再卡住（重点回归「卡住了.png」场景）；② 排队消息不再消失，新会话能正常收到回复；③ 选 sonnet 时实际调用的确是 sonnet（非 opus4.8），meta 行模型名与所选一致；④ 发送带附件的消息后，时间线用户气泡能看到附件 chip；⑤ 网络瞬断重试时状态行出现「Network retry N/M」可见提示，不再是无声等待；⑥ combobox 下拉不再横向滚动；⑦ textarea 不再可拖拽变形、多行文字垂直居中；⑧ Model/Effort 选择器触发器尺寸一致不再挤压；⑨ 卡片内边距上下对称；⑩ 时间线顶部滚动时有柔和淡出（贴底滚动场景属固有行为非 bug）。
0-novies. **T-30 批2 第五轮点验（2026-07-31 新增，`4735a45`——形态 + 字体一并点）**：① follow-up 卡满圆 pill、静息高 42px、聚焦边框**不再变橙**（中性灰一步加深）；附件 chip/notice 出现时卡自动变回 12px 圆角不变形；② Model/Effort 已合并为一枚 `Sonnet High ⌄`：静息无框无底，悬停/Tab 聚焦/弹开三态显底壳；弹层两组（Model / Reasoning effort）各单选；选 Default 后缀整段消失；选择重启后保持、发送实调所选模型（回归 b159e4a 模型直通）；③ 圆形发送键 24px 近黑（运行中 Stop 仍红），同屏最多一件高饱和圆；④ ⊕ 钮在卡最左：点击 = 光标处插 `@` 并弹文件搜索（**不是附件上传**，悬停文案 Add file context）；⑤ empty 态不再常驻 `Ready · cwd:` 行，cwd 改为悬停文件夹下拉可见（title）；发送中/读附件时状态行照常出现；⑥ 队列条/问答卡与 Composer 卡净空目测 8px（原 14px）；⑦ 字体分域：正文/侧栏/按钮为比例字体；代码块/路径/hash/工具行**路径型**参数仍等宽；Bash 描述、「2 files, 3 searches」、「Worked for Ns」为比例（若这三处出现等宽即 argKind 回归）；⑧ 状态栏计时/token 计数原地刷新**无横向抖动**（tabular-nums）；⑨ 侧栏分支 chip 13px 比例字体、标题可见字符数变多；阅读栏略窄（48→45rem）；⑩ **a09 采集**：三测点（阅读栏中英混排/侧栏树/工具行混排）各截 mono（DevTools 给 html 加 `data-font-domain="mono"`）与 split 两张 + Cursor 参照裁片，落 `docs/design/refs/a09-font-domain/`，D25 量化八项照 [`a09-font-domain-baseline.html`](../../../design/a09-font-domain-baseline.html) 表回填；`lowercase` 六处豁免同批目视。
