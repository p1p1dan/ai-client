...
1. **T-29 Markdown 渲染**（D26）：assistant 正文 react-markdown+shiki，chat 专属映射零 XSS 面，流式期纯文本完成后切。→ 执行计划 §3 T-29 行。**✅ 代码结项 2026-08-04（`d320206`+`666c7c3`+`4507df3`）——双轨对抗复核闭环（27 findings / 3 证伪 / verify:security 补格 / Codex 双盲），关注面 (a)~(e) 全裁定；三绿 1901 例 + 产物 20 shiki chunk 白名单内；待用户 GUI 点验 0-duodecies（用户线）后转 Done。明细见[主线台账](../../../plans/ledger-claude-mainline.md) 2026-08-04 T-29 行**
...
0-duodecies. **T-29 Markdown 渲染 GUI 点验清单（2026-08-04 新增；含拍板两项）**——首轮反馈「展示效果都不错」+ 1 缺陷（内容不可选中）已修 `b08f6ae`，①~⑪ 待正式逐项表态：
