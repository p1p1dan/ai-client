...
- **Last Landed**:
  ...
  - 文档（2026-07-29 三批）：**A07 中列 Cursor 观感基线(已升 v3,实质定稿待用户最终过目)** [`docs/design/a07-cursor-composer-alignment.html`](../../../design/a07-cursor-composer-alignment.html)——六屏 × Flexoki 亮暗;§08 五件事已全部裁定,工具行交互口径(Read 可点击/Grep 悬浮命中列表)与 main/master chip 已烧入;定稿后重写 T-05 验收标准，token 与 globals.css 机器校验一致；**待用户浏览器验收**（重点：Tab 栏带/不带、运行位置只读 chip），通过后 T-27/T-28 方可施工
