...
- **Last Landed**:
  ...
  - 代码（2026-07-31 二批）：**T-30 批2 施工落地（D25 分域字体 + Composer 形态对齐 Cursor，S0~S7 全序）** —— 断言先行 18+ 条（fontDomainGuards A1~A6 / composerFormClasses F-A1~A22）→ `globals.css` 分域两栈 + 五新 token + A/B flag → 形态主体（42px pill 卡 + 中性聚焦梯度 + 24px 单一控件档 + **合并 `Sonnet High ⌄` 控件（删 ModelSelect/EffortSelect）** + ⊕ 接 @ 文件搜索（T-28 偏离①结清）+ 8px 净空归一 + empty 状态行按需显隐）→ `argKind` 双域工具行参数 + `<Ident>` 原语 → chat/workspace-shell 域映射全量清零（text-xs 等四类禁用字号）→ A07 v4 追记一~十一节一次成文 + phase0a/design-system 同批改写 → a09 凭证页骨架。三绿：typecheck 干净 / biome src 0 诊断（a08 外来诊断豁免，本轮实测含 5 errors 亦在其内）/ vitest 76 文件 **1403** 例（+58，3 例 Windows-only 豁免不变）。红线 `chatSessions.ts`/`runtimeEvents.ts` 零改动。**残留：a09 三测点截图 + 量化八项 GUI 实测随第五轮点验**。明细见主线台账 2026-07-31「T-30 批2 施工落地」行
