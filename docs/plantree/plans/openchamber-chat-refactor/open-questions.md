...
30. **历史断链后 resume 句柄语义（xvqiu1-2 连带）**：`jsonl_not_found` 后 Host 保留 `runtimeIdentity`，下一 send 仍带 `resume:`（`claudeRuntime.ts:752`）——捆绑 CLI 源码判读对未知 id exit 1 → `session.failed`，报错卡「会话未中断，可以继续发送消息」承诺为假。**判读未经真机取证（Host log `[cli-stderr]` 一看便知），确认前不施工**。确认后拍板：清句柄静默 fork（改卡文案）vs 显式按钮 opt-in。连带一并裁：索引条目加 `projectsRoot?` 标记与跨目录徽章 · 卡内「归档这条」入口（复用 `setArchived`）· `host.ready.settings` 要不要含 configDir（现报错卡 Details 在漏路径而 host.ready 脱敏不发，口径不一致）。环境因与修法全文见 [xvqiu1 triage §2](../../../plans/2026-08-11-xvqiu1-triage.md)。
31. **流式状态行三小件（xvqiu1-4 连带）**：主修复路径无需拍板（见 [xvqiu1 triage §4](../../../plans/2026-08-11-xvqiu1-triage.md)）；待定：① token 计数口径（仅 ↓ 输出 tokens 对齐官方 CLI vs 复用 StatusLine 的 in/out/ctx%，后者恐重复）；② 趣味动词轮换要不要 + 中英语言（与 multi-agent open-q #11 权限卡文案语言一并裁）；③ test.4 出包 flag 默认位（建议 ON 让真机真正踩到流式路径，CI 保 OFF 位绿）。（spike 补正 2026-08-11：`message_delta.usage` 每消息一次收尾阶跃、非逐 token 游标，回合累计需 Host 自加；`thinking_tokens` 与 partial 无关今天就在发、token 计数可先行——见 [spike 报告 §5](../../../plans/2026-08-11-partial-messages-spike.md)。）
32. **buchong1 参考批（ZCode 九图）采纳范围**（2026-08-13）：细节对照、候选 S/M/L 分档与证据全文见[分析档](../../../plans/2026-08-13-buchong1-zcode-reference.md)。待拍板：① **Progress 面板**（用户明示「很喜欢」；survey 证实 Claude 侧 TodoWrite 结构化 todos 今天就随 tool_use.input 进 renderer——T12-15「不做 Plans（无数据源）」裁定的事实前提已过期，采纳即推翻该裁定（D27 复核维持过）须明示；Codex 侧无等价协议需显式降级；数据完整性留实现方施工前实测确认）；② **per-session 权限模式选择器**（协议类型 `SessionPermissionMode` 全套已定义、Host 两侧硬编码、IPC 无字段——纯接线 L 档，触 Host 红线须 flag+Happy Path）；③ **S 档六件**（代码卡头部行 / header chip / 选项 description / thinking 内滚动 / diff stat 绿色 / busy spinner）是否直接开小批——全部纯渲染端零协议改动；④ **会话右键菜单**（连带收口 #6：现状右键=立即归档无菜单无确认，`LeftNav.tsx:756-760`，属真实误触面；用户明示菜单项取子集即可）；⑤ 冲突项（气泡右对齐 / 工具行图标 / 模型-effort 双下拉 / Group-Project 互斥切换）默认不采纳，除非明示推翻 D26④ / D24 / T-30b2。
