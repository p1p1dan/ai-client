...
- **T-31 回复解剖 + 置顶气泡（第四轮点验第 6/7 条）**：代码已落 `8109d45`（2026-08-03，36 文件 +4712/−318，测试 1593→1759）——回合层 / 状态读条迁回合头（owner token + 基线 × `h:` 双守卫）/ 折叠壳（授权强制展开红线）/ CSS sticky 置顶气泡 + scroll-state 截断 / 尾部条 copy / D26④ 满宽补落 / §4.5 撤回；Opus+Codex 双轨对抗复核 + 修复批三轮 + Codex 终验两轮全闭环；A07 v5 追记。「impl done 待 GUI 联调」不算 Done——点验清单见 [implementation-status 用户线 0-undecies](./implementation-status.md)，通过后转 Done。明细见[主线台账](../../../plans/ledger-claude-mainline.md) 2026-08-03 T-31 行。
