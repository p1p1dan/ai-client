...
- **T-29 Markdown 渲染**：🟡 **代码已结项**（`d320206`+`666c7c3`+`4507df3`，2026-08-04；前两 hash 曾落库未登记，本日补登）——双轨对抗复核闭环（A 轨 Workflow 27 findings / 24 verdicts / 3 证伪 + verify:security 独立补格 + Codex 双盲收活），修复批根治 major「新回合抹掉全会话历史回答 Markdown」并把关注面 (c) 收窄为逐消息即时转正；三绿 1901 例 + 产物 20 个 shiki 语法 chunk 全在白名单；红线零改动。按本表口径**待用户 GUI 点验（清单 0-duodecies，含拍板两项：脚注 13px / 六级标题三档）后转 Done**。明细见[主线台账](../../../plans/ledger-claude-mainline.md) 2026-08-04 T-29 行。
