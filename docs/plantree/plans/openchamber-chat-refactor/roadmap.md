...
- **T-30 观感打磨批1（快赢，D25/D26）**：12 项快赢中 10 项已落 `3dcd2dc`（2026-07-30，6 文件 +95/-40；P-19/P-22 让给 T-23）。「impl done 待 GUI 联调」不算 Done——点验清单见 [implementation-status 用户线 0-septies](./implementation-status.md)，通过后转 Done。**批2（D25 分域字体 + Composer 形态对齐 Cursor）代码已于 2026-07-31 施工落地**（S0~S7 全序：18+ 条断言先行 / 分域两栈 + 五新 token / 42px pill 卡 + 24px 单一控件档 + 合并 `Sonnet High ⌄` 控件（删 ModelSelect/EffortSelect）+ ⊕ 接 @ 文件搜索 / argKind 双域工具行 / 域映射全量清零 / A07 v4 追记一次成文 / a09 凭证页骨架；三绿 1403 例 +58）——「impl done 待 GUI 联调」不算 Done：**a09 三测点截图 + 量化八项 GUI 实测 + 第五轮点验待用户**。明细见[主线台账](../../../plans/ledger-claude-mainline.md) 2026-07-31「T-30 批2 施工落地」行。
