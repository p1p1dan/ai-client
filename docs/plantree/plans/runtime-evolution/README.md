...
| — | H / 21 | [外部 Agent 迁移](topics/external-agent-migration.md) | 排期未定：P0（错误文案指向迁移入口）与 P1（老用户首启一键迁移）属于 H / 19 的收尾，应尽早；Claude / Codex 各层要先定分层与 P5-4 归属 |
...
| H / 21 | 外部 Agent 迁移（默认迁移策略 + Claude / Codex） | ⬜ **待决策，未开工**。两件事：老用户首启默认迁移到什么程度；从 Claude Code / Codex 迁移配置与对话做到哪一层。对话导入可能落在既有 P5-4；[实施计划](topics/external-agent-migration.md) |
