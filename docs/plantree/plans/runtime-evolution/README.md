...
**最近落地**：`76424efa` native shell/ESM bundle/双后端打包门禁，`3d14a4b2` 固定事件录制分块，`2397f1ed` 交付文档；另已引入主线 GUI A～E（当前分支 `d7def5c3`～`80294b63`），[来源映射与证据](evidence/p4-6/README.md)。
**最新验证**：[CI 34306438655](https://github.com/p1p1dan/ai-client/actions/runs/34306438655) 完整门禁与 Linux 双后端产物通过；Windows 两后端正常 dispose 后强退触发 libuv 断言，已改为 Node IPC 关闭后自然退出，待新 CI 复验。该 CI 不含主线 GUI A～E。
**下一目标**：手动 CI 生成包含主线 GUI 与退出修复的 **1.0.0-test.11** → P4-6 Windows/企业加密机累计签收，另带 [GUI 清单](../gui-sdk-experience/现场验收清单.md)。不本地打包、不推 tag。P2-5/P2-6 真实缓存门槛仍为 95.01%，P5 尚未开工。
