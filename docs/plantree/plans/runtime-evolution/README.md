...
| — | H / 21 | [外部 Agent 迁移](topics/external-agent-migration.md)、[对话导入](topics/conversation-import.md) | 进行中。P0（错误文案指向迁移入口）`22da278c`、P1（老用户首启一键迁移）`f147059b` 已落地，迁移页不会再动，可与 H / 19 剩余界面点验一次做完；剩下对话导入 C1～C6 一摊，开工前先核对与 P5-4 的归属 |
...
| H / 21 | 外部 Agent 迁移（默认迁移策略 + Claude / Codex 对话导入） | 🟡 P0 错误文案 `22da278c`、P1 首启一键迁移 `f147059b` 已落地；对话导入 C1～C6 未开工。两项决策已定（首启一键确认；只做对话导入、可看可续聊）；[策略](topics/external-agent-migration.md) · [对话导入施工计划](topics/conversation-import.md) |
