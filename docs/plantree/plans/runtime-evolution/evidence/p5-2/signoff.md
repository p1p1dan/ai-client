...
| SA09 长任务与 maxTurns | ✅ tested | `run.ts` 的 `afterToolCall` | 假时钟越过旧 idle/duration 不杀 Agent；`bash` 显式 `timeoutSeconds`；maxTurns 截断有报告与 `truncated` 状态 |
...
| SA12 模型与 thinking | ✅ tested | `index.ts` `resolveModel` | Task override > pin > parent；不可用即失败并列出可选项，**绝不回落**；密钥不进事件也不进定义 UI |
...
| SA15 usage 与报告限额 | ✅ tested | `subagentHistoryUsage` / 12k / 50k | 父 provider 原数不变、子成本恰计一次；报告 12k、合并文本 50k、历史摘要 4k 分别限额；截断不丢终态 |
...
| SA19 旧插件/定义迁移 | ✅ tested | `subagent/migrate.ts`、`nativeSubagentSettings.ts` | legacy 与 native 互斥、旧开关语义保留；预览显示逐字段差异、冲突阻断、原文件不覆盖、项目定义不自动提升 |
