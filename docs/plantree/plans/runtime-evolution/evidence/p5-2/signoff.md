...
| SA09 长任务与 maxTurns | ⚠️ 与代码不符（审计 subagent-core-16，仍未修，T027 只改写此行措辞） | `run.ts` 的 `afterToolCall` | ~~假时钟越过旧 idle/duration 不杀 Agent~~——`idleTimeoutSeconds`/`maxDurationSeconds` 仅解析、从未被任何计时器读取，是 `run.ts` 模块注释明写的设计移除（"The watchdog fields are gone, not disabled"），契约 [p5-2-subagent-contracts.md](../../topics/p5-2-subagent-contracts.md) 第 45 行同样写明「不启用 idle/duration 杀子任务」——这不是靠假时钟测试证明的行为，是没有计时器可测；`bash` 显式 `timeoutSeconds`；maxTurns 截断有报告与 `truncated` 状态；**审计 2026-09-14**：仓库里没有任何子代理假时钟测试（`grep -rl useFakeTimers src/runtime` 零命中），原「✅ tested」标注对不存在的计时行为声称有测试佐证，确属夸大（subagent-core-16） |
...
| SA12 模型与 thinking | ✅ 已修（T006，`d71eb2ec`，2026-09-14）：`resolveModel` 第三档改读 `bindRun()` 传入的父回合已解析 model ref（`this.runContext?.model`），校验其仍在当前目录里才使用，找不到时报错并列出可选项；`adapter.defaultRef()` 现在只在从未调用过 `bindRun()`（如独立探针）时才触发。原审计标注保留于下 | `index.ts` `resolveModel` | Task override > pin > parent；不可用即失败并列出可选项，**绝不回落**；密钥不进事件也不进定义 UI；**审计 2026-09-14**：`resolveModel` 第三档实际取 `adapter.defaultRef()`（目录第一个 provider 的第一个模型），并非父回合模型，"绝不回落"与实测不符（cross-01） |
