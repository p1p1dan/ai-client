...
| P3-1 | ✅ | 有缺口 | close() 永久 reject；写锁 TOCTOU（待定）。均已修（T015，`ce2a7af8`；TOCTOU 用确定性用例证实成立）。批评者容量缺口：对账表与 MCP / trace 限额已落（T024，`8564ba41`），子代理转录限额（T020，`60b250f3`） |
...
| P5-2-0 | ✅ | 未评估 | — |
| P5-2-1 | ✅ | 有缺口 | 定义不按 run 重读；pin 诊断无出口。cross-01/02 父模型与档位继承已修（T006，`d71eb2ec`）；其余已修（T020，`60b250f3`） |
