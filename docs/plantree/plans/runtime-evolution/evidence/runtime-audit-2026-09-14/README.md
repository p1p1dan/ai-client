...
| P3-4 | ✅ | 有缺口 | 报告当用户消息（high）、子代理审计行、空 assistant、无重试事件。前两项已修（T005，`4e80f9ff`）；后两项归 T017 |
...
| H/20 | 🟢 | 有缺口 | seq 陈旧（high）、压缩锚点、CLI 补换行（待定）。已修（T003，`0332214c`）；CLI 补换行判可达并修尾片形态，中段形态见 Q008 |
