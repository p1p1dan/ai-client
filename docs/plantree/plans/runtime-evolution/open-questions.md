...
- **`pi-coding-agent` 按什么口径算「摘除」**：**用户选口径 A——保留为随包可执行文件，收回库角色。**
  成功标准第 3 条写「从 package.json 移除」时它只是一个库；后来 H/18～H/20 让它又成了
  内嵌 Pi 终端与插件管理的执行体，按字面删会连带下线这两项功能，还会让 P6-4 的回退开关
  没有回退对象。落地形态：应用代码里除旧引擎外一处不 import 它（守卫 `piCliIsBundledToolOnly.test.ts`
  越界即点名），native worker 进程实测零加载；终端与插件管理只通过进程用它的 `cli.js`。
  第 3 条按实质达成签收，见 [ARD 成功标准](../../../plans/2026-09-08-runtime-evolution-ard.md#7-成功标准)
  与 [P6 施工计划](topics/p6-cutover.md)。

已于 2026-09-12 关闭：

- **H / 17 是否已覆盖 P5-5 模型目录切源**：**没有覆盖**，第 9 批按两个节点做。
  判据是探针实测而非读文档——往目录里放四个 provider 让自有 runtime 读一遍，
  `mistral-conversations` 与 `azure-openai-responses` 两个用户服务被**静默丢弃**。
  三条缺口各自独立：风格覆盖面（H/17 开放十种、runtime 只认四种）、
  D15 一行未实现、以及 H/17 其实是**加深**了对派生明文文件的依赖，
  而 P5-5 要做的正是把 native 从那份文件上摘下来。
  见[施工计划](topics/p5-4-p5-5-import-and-catalog.md)与[记录](evidence/p5-4-p5-5/README.md)。
