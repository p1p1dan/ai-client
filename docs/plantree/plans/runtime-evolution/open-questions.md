...
| 老用户迁移的默认程度：完全静默 / 首启一键确认 / 维持现状 | H/19 点验实测：迁移没跑之前旧会话一律 resume 失败。分歧只在静默到什么程度——静默复制资源没有争议，**静默把 API key 吸进本应用 vault 是同意边界问题**。三个选项与代价见[外部 Agent 迁移](topics/external-agent-migration.md#决策待定一老用户要不要默认迁移)，建议选项 B | H / 19 / H / 21 |
| Claude / Codex 迁移做到哪一层 | 用户 2026-09-10 提出支持 Claude、Codex 对话迁移。已实测两边的格式：Claude 是 `projects/<slug>/<uuid>.jsonl`、消息按 `parentUuid` 成树（拍平会丢分支）；Codex 的历史有相当部分在 sqlite 里，形状完全不同。**读出来 ≠ 能接着聊**。另有一条方向上的不同意见：新用户的真杠杆可能是配置而非历史。分层建议见[外部 Agent 迁移](topics/external-agent-migration.md#建议的分层待用户确认) | H / 21 |
