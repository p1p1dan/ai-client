...
| 默认字体用 Maple Mono 的范围 | 用户希望默认使用 Maple Mono Normal NF CN（SIL OFL，每字重约 17MB，安装包每字重约 +9MB）。它是等宽字体、中文按 2:1 占格，整界面统一时中文显得字距偏宽；已出三组对比截图（当前 / 全界面 / 只替换等宽域）。需定范围：全界面、只等宽域加终端编辑器，或只终端编辑器；这决定是否撤销 D25 的比例栈 | 设计规范 D25 |
