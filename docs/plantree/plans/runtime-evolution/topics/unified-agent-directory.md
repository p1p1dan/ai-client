...
- [ ] U1：agent 目录解析改为两种模式一致；删除 H/17 引入的条件分支。
- [ ] U2：迁移能力（复制）——skills、`AGENTS.md`、模型服务导入 vault 用户组；明确的确认步骤，冲突时不静默覆盖。
- [ ] U3：会话目录随 agent 目录统一。用户 2026-09-10 决定不管用户自己终端里的官方 `pi` 能否找到，只保证**本应用内**的 GUI 与 TUI 能找到历史对话；官方支持等其后续版本。
- [ ] U4：插件界面——列表、安装、卸载、启用开关，全部调 pi 的 install/remove/list；同时显示当前审批用的是随包还是用户自配的权限系统。
- [ ] U5：迁移完成后移除借用机制与相关 env。
- [ ] U6：验证。
