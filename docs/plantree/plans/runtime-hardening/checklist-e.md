...
| DEV-36 | cutover-03：native 下「能力」面板与插件页文案（判据 2026-09-17 拆成两行，见 5.1；页面路径 2026-09-18 改了，见 5.1 末行） | 侧栏「能力」面板列出 native runtime 自己的 MCP / 技能 / 子代理（「未报告」与「报零」两态可区分）；插件设置页权限归属文案：① 无条件显示「本应用的每个对话，都由它自带的权限系统审批工具调用」；② 装了 pi 权限扩展时另显示「你自己安装的 pi 权限系统只对内嵌终端生效」。**两句文案本身未改，只是所在页面从「设置 → Pi」挪到「设置 → 扩展」** | 起 Electron 截两处图（T026 `2cfed556` 已改实现，这里验的是用户看到的样子）；插件页现在开在**设置 → 扩展**（第一节就是「插件」）；②要先 `pi install` 一个权限扩展再看，开发机 2026-09-17 未装、只验到 ① | cutover-03 |
...
| MODEL-28 权限卡倒计时 | 「不响应满 **119 秒**后自动拒绝」 | 「等满 **120 秒**」 | 常量是 `PERMISSION_TIMEOUT_MS = 120_000`（`src/runtime/plugins/permissions/index.ts:18`），119 秒来源不明；按 119 秒判会在临界点误判成「没到点就拒了」 |
| ENC-21 TSD 魔数误判 | 「手写一个以 `%TSD-Header-###%` 开头的明文 txt，read 它」 | 造文件必须**同时**满足两条：前 16 字节是魔数、**总大小 ≥ 4096 且为 4096 的整数倍**；另造一份 4000 字节的对照组 | `containerShaped()`（`src/runtime/host/io.ts:41-44`，判定处 `:152-155`）要求块对齐，只写魔数不对齐的文件会被正常读成明文。照原判据字面做会得到「不报错」，极易误记成「误判已修」 |
| ENC-23 / ENC-24 helper 相关两项 | 「helper 的 stderr 噪声不串进正文」「大文件读时 helper 进程数是个位数」 | 以 ENC-16 结果为前提分支记录：**ENC-16 通过（读到明文）时 helper 根本不会被调起**，两项平凡满足，照实记「helper 进程数 0 + 走直读路径」并写明前提；只有 ENC-16 判负、真的落到回落读时，这两项才有可观测对象 | Windows 打包态 worker 载体是写死的随包 node.exe（`PiWorkerProcess.ts:78-92`），helper 只在 worker 自己读到密文时才起；且 helper 环境被固定剥掉 `NODE_OPTIONS`（`io.ts:63-67`），没有产品级办法让它往 stderr 打噪声 |
| WIN-8 托管模式 + 钥匙串锁定 | 「锁定钥匙串后点三个按钮」 | Windows 上**没有**「钥匙串锁定」这个状态（DPAPI 随登录会话，锁不上）。改用等价造法：备份后弄坏凭据库 payload，逼出同一条「凭据不可读 → 不下发 modelCatalog → worker 回落读 agent 目录」的路径，并在现象里**写明用的是等价造法** | 判据措辞来自 macOS 钥匙串；照字面在 Windows 上无法构造，写不出结论会被读成「没做」 |
| MODEL-26 archive 分代拒绝 | 「升级 `RUNTIME_CONFIG_VERSION` 前后各真实采集一次」 | 改记 **⛔ 不可执行（打包态）**：要跨一次代码升级重新构建两个产物，装好的安装包上无法构造；留待有两个相邻版本产物时另行安排 | 判据本身需要改代码并重新打包，不属于「在一台装好的机器上能做的事」 |
| ENC-1 / MODEL-4 / MODEL-30 的「载荷 / 次数」半边 | 「量 `utility.start` 发出到 ack 的耗时」「抓 `utility.start` 载荷」「抓 IPC 调用次数」 | 三处都改成可观测的替代量：ENC-1 记「点击 → 出结果」的上界并注明 10 秒请求超时（`WorkerSlot.ts:92`）未触发；MODEL-4 只记回执里的模型名；MODEL-30 记界面上的数字并说明 preload 对象被冻结、打不上计数补丁 | `PiUtilityService.ts:190-199` 不打日志，utility 链跑在 `traceDir: null`（`nativeUtility.ts:154`），trace 里也没有这三个量；打包态没有 dev server，渲染层也挂不上探针 |
