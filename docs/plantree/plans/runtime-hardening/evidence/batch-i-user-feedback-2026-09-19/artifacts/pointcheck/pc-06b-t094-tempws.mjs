...
// After the restart the app's workspace really is the temp directory
// (`--open-path=/tmp/pc-i/ws` took effect this time), so the scratch files are
// two levels deep instead of nine and nothing inside the repo is touched.
const DEMO = '/tmp/pc-i/ws/sub';
/** name → depth, counting the workspace root's children as depth 0. */
const PATH_PARTS = [['sub', 0]];
const FILE_DEPTH = 1;
const DEMO_DIR_NAME = 'sub';
...
const PANEL = `(() => {
  ...
  // The temp workspace has no node_modules/package.json to key on, so pick the
  // scroller that lists this workspace's own top-level entries.
  return vps.find((v) => /README\\.md/.test(v.innerText || '') && /sub/.test(v.innerText || '')) ?? null;
})()`;
...
try {
  ...
  const dr = await rect(DEMO_DIR_NAME, 0);
