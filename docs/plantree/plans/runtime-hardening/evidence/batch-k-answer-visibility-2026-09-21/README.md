...
- [x] T110：审阅图标打开中列工作区 diff；实现/验证/提交。
...
- 提交前基线：T109 `5c7c7596`。
- 改动：审阅条目图标调用实际 editor store 的 `openDiffTab`，随后调用面板 `onClose`。重复打开同一路径也会关掉审阅，不依赖 activeTabPath 必须发生变化。
- 必要适配：交接示例省略的 `staged` 在实际类型中必填，使用 `false`；审阅状态 `added/modified/unknown` 转为 Git 显示码 `A/M/undefined`，不强行类型断言。审阅原始路径是绝对路径，Git `:path` / `HEAD:path` 要求工作区相对路径，因此通过小型纯函数复用现有路径解析器，统一 POSIX / Windows / UNC 路径；工作区外文件沿用已有错误提示，不创建误导性整文件新增 diff。
- `onShowFiles/filesOpen` 没有退役：面板顶部的“文件”切换按钮仍是有效消费者；条目图标已经不再使用该链，也不再发送 fileOpenIntent。保留顶部按钮并有点击回归验证，符合“仅失去最后一个消费者时删除”的条件。
- 实际行为：中列 `git-diff://` 页签展示当前工作区差异，聊天区让位；面板内 patch 仍是单次记录。A2 未实现。
- 反向验证：更新后的交互断言在旧打开文件实现下失败。
- 小批测试：sessionReviewInteraction 1、sessionReviewDiffTarget 4、diffCenterTabsStatic 13（18）；diffTabTarget 16、centerLayoutModel 47、i18nCoverage 2（65），共 83 条通过。真实组件 DOM + 实际 editor store 验证 tab 地址、target、重复复用、关闭回调、按钮不触发展开和顶部文件入口。
- 三套 tsc 均退出 0；改动文件 Biome 通过。
- 真实 GUI 跳转截图未取得：按用户对测试窗口的反馈，后续没有重新启动无凭据测试实例。不得把 store/静态测试写成 GUI 验收通过。

## 收口与交接

- 四项分别提交：T107 `678d79b7`、T108 `3a0c13fa`、T109 `5c7c7596`；T110 与本段同一提交（标题 `fix(review): T110 从审阅条目打开中列工作区差异`）。未推送，未触发 CI。
- 原选型演示页归档副本：[折叠与 diff 落点](aiclient-preview-answer-fold-and-diff-open.html)、[正文全露三方案](aiclient-preview-answer-visible-3ways.html)。原 `/tmp` 文件保留，避免清掉其他会话的演示材料。
- 实现/提交已完成不等于完整验收：真实 ≥20 次工具调用长回合、≥100 字符命令回合、真实 ≥10 条审阅记录与键盘操作、中列 diff 截图均待补；T111 中断取证也待可复现会话。
- 本机全量门禁未跑，CI 是权威；本轮没有 CI 通过结论。
- 测试隔离 profile 为 `batch-k-check`，无用户登录态；出现“未登录/未拿到模型凭证”是此测试环境条件。用户反馈后已关闭应用和探针，没有修改用户原登录配置。
