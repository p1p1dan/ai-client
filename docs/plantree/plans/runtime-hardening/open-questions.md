...
| Q033 | `updaterChannel.test.ts` 的反例「1.0.0-test.* 用派生通道仍复现 No published versions」在 electron-updater 6.7.3 下不成立（真实 `GitHubProvider` 解析到了 v1.0.1）。是测试写错了，还是 `486855f9` 对「测试版检查更新失败」的根因推断（`allowPrerelease` 按渠道匹配 tag）本身不对？ | 2026-09-24 本机全量测试稳定复现（与 `feat/ctrl-enter-interject` 无关，main 上即存在；本机 electron-updater 版本与锁文件一致）；测试只替换了 feed 与网络，走的是真实 provider | [T124](roadmap.md)（发版门禁若跑全量会被卡） |
