...
| Q003 | 「本次会话允许」授权是否按委派归属收敛？ | 现在 grantKey 只有 tool/path/command/paths，子代理批一次父代理也免卡（cross-05）；按钮文案字面就是会话级 | T002 |
| Q004 | 项目 `.agents/skills` 是否照 pi 一路找到仓库根？ | pi 文档写「cwd 及祖先目录到仓库根」，本仓只看 cwd 一层（skills-mcp-22，K1 验收未写祖先目录） | T019 |
| Q005 | session-02（CLI 打开文件补换行毁掉尾片修复）是否真实可达？ | 两侧代码事实已核实，缺一条组合用例 | T003 |
