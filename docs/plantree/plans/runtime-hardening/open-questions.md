...
| ~~Q033~~ | `updaterChannel.test.ts` 的反例在本机不成立：是测试写错，还是 `486855f9` 的根因推断不对？ | 2026-09-24 本机全量测试稳定复现 | 已结案（2026-09-24）：测试写错，根因推断成立。测试与 `GitHubProvider` 各用一份 `semver`（嵌套副本），跨实例 `instanceof` 失败使 provider 读不到预发布段；改用 provider 自己的 `semver` 后反例稳定复现 `ERR_UPDATER_NO_PUBLISHED_VERSIONS`。见 [T124](roadmap.md) |
