...
- 同步开发主线 GUI A～E：目录菜单与终端设置、新会话沿用目录、真实运行/重试状态、授权详情折叠、问答与文件 diff、上下文用量及输出贴底跟随。
- Windows Node worker 正常 dispose 改为关闭 IPC 后自然退出，避免强制退出时的 libuv 断言；待 CI Windows 复验。
