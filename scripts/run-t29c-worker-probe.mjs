...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'aiclient-t29c-runner-'));
...
try {
  ...
  await esbuild.build({
    entryPoints: [path.join(repoRoot, 'scripts', 'probes', 't29c-worker-probe.ts')],
    outfile: entry,
    bundle: true,
    platform: 'node',
    format: 'cjs',
  ...
  if (result.status !== 0) {
    throw new Error(
      `T29-c Electron probe failed (status=${result.status} signal=${result.signal})\n` +
        `${result.stderr}\n${result.stdout}`
    );
  }
  ...
  if (
    report.ok !== true ||
    report.explicitTerminal !== 'session.stopped' ||
    report.streamed !== true
  ) {
    throw new Error(`invalid T29-c probe report: ${JSON.stringify(report)}`);
  }
