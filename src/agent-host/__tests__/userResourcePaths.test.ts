...
describe('hasBorrowedPaths', () => {
  it('is false only when both lists are empty', () => {
    expect(hasBorrowedPaths({ skills: [], promptTemplates: [] })).toBe(false);
    expect(hasBorrowedPaths({ skills: ['/a'], promptTemplates: [] })).toBe(true);
    expect(hasBorrowedPaths({ skills: [], promptTemplates: ['/b'] })).toBe(true);
  });
});
