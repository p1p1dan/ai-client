...
import { prewarmWindowsRegistryEnvironment } from './services/terminal/PtyManager';
...
app
  ...
  .then(async () => {
    ...
    void cleanupTempFiles().catch((error) => {
      console.warn('[startup] Failed to clean up temp files:', error);
    });
    void migrateLegacyTodoIfNeeded().catch((error) => {
      console.warn('[startup] Failed to migrate legacy todo data:', error);
    });
    void checkGitInstalled()
      ...
      .catch((error) => {
        console.warn('[startup] Failed to check Git installation:', error);
      });
    void prewarmWindowsRegistryEnvironment().catch((error) => {
      console.warn('[startup] Failed to prewarm Windows registry environment:', error);
    });

    mainWindow.once('ready-to-show', () => {
      void autoStartHapi().catch((error) => {
        console.warn('[startup] Failed to auto-start Hapi:', error);
      });
    });
