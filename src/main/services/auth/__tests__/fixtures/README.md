...
| 项 | 值 |
|---|---|
| 日期 | **2026-08-26**（codex 0.145.0 → 0.149.1 升级票，三件套之①） |
| codex-cli 版本 | `0.149.1`（**随包那份**：`src/agent-host/node_modules/@openai/codex-linux-x64/vendor/x86_64-unknown-linux-musl/bin/codex --version`，不是开发机全局的） |
| 命令 | `CODEX_HOME=<tmp> AICLIENT_CODEX_API_KEY=blessing-spike-dummy-key <随包 codex> --strict-config doctor --no-color` |
| 生成器输入 | `generateManagedCodexConfigToml({ baseUrl: 'https://cch-blessing.example.com/v1' })` |
| 生成字节 | 与本目录已提交的 `codex-config.blessed.toml` **逐字节相同**（本次没动生成器，故 fixture 无需替换——重跑验的是「新 codex 还接不接受这串字节」） |
| 结果 | `config.toml parse ok`；无 `unknown configuration field` / strict-config 拒绝；`auth` ✓ 且 `provider auth env var AICLIENT_CODEX_API_KEY (present)`；`sandbox` ✓ `restricted fs + restricted network · approval OnRequest`。仅 `reachability` 失败（fake base URL 连不通，预期内，与 strict-config 无关），doctor 因此 exit 1 |

> ⚠️ `CODEX_HOME` 别指向 `/tmp`：0.149.1 的 doctor 会拒绝在临时目录下建 helper 二进制
> （`Refusing to create helper binaries under temporary dir`），并且 `disk` 检查要求
> `CODEX_HOME` 所在卷有 ≥5 GiB。两条都与 strict-config 无关，但会把输出搅浑。

### 历史记录

| 日期 | codex-cli | 结果 |
|---|---|---|
| 2026-08-15 | `0.145.0` | 首次 blessing：`config.toml parse ok`，仅 `reachability` 失败（预期内） |
...
import('<repo-root>/src/shared/codexManagedConfig.ts').then(m => {
...
# Confirm no "unknown configuration field" / strict-config rejection. If the
# generator changed, copy the newly generated bytes over the fixture:
