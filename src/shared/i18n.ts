 ...
 *
 * A bash grant reaches further than the command on the card (every command with
 * the same prefix) and a file grant reaches exactly one file, so the card has to
 * say which, or the button promises something the user cannot see. Its own block
 * at the end of the file for the usual reason: several people add strings here
 * on the same day.
 */
Object.assign(zhTranslations, {
